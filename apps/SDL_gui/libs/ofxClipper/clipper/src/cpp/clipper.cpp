...
#if defined(_MSC_VER)
__declspec(noreturn) extern void msvcfatal() {}
#endif
  ...
  public:
    ...
    Int128 operator/ (const Int128 &rhs) const
    {
      if (rhs.lo == 0 && rhs.hi == 0)
          //return; //throw "Int128 operator/: divide by zero";
#if defined(_MSC_VER)
          msvcfatal();
#else
          return 0;
...
void RangeTest(const IntPoint& Pt, bool& useFullRange)
{
  if (useFullRange)
  {
    if (Pt.X > hiRange || Pt.Y > hiRange || -Pt.X > hiRange || -Pt.Y > hiRange) 
        return; //throw "Coordinate outside allowed range";
  }
...
bool ClipperBase::AddPath(const Path &pg, PolyType PolyTyp, bool Closed)
{
  ...
  if (!Closed && PolyTyp == ptClip)
      return; //throw clipperException("AddPath: Open paths must be subject.");
#else
  if (!Closed)
      //return; //throw clipperException("AddPath: Open paths have been disabled.");
#if defined(_MSC_VER)
    msvcfatal();
#else
    return 0;
  ...
  while (highI > 0 && (pg[highI] == pg[(size_t)highI -1])) --highI;
  if ((Closed && highI < 2) || (!Closed && highI < 1)) return false;

  //create a new edge array ...
  TEdge* edges = new TEdge[(size_t)highI + 1];
...
Clipper::Clipper(int initOptions) : ClipperBase() //constructor
{
  ...
  m_UseFullRange = false;
  m_ReverseOutput = ((initOptions & (int)InitOptions::ioReverseSolution) != 0);
  m_StrictSimple = ((initOptions & (int)InitOptions::ioStrictlySimple) != 0);
  m_PreserveCollinear = ((initOptions & (int)InitOptions::ioPreserveCollinear) != 0);
  m_HasOpenPaths = false;
#ifdef use_xyz  
  m_ZFill = 0;
#endif
    ...
    PolyFillType subjFillType, PolyFillType clipFillType)
{
  ...
  if (m_HasOpenPaths)
      //return;  // throw clipperException("Error: PolyTree struct is need for open path
      //           // clipping.");
#if defined(_MSC_VER)
      msvcfatal();
#else
...
void Clipper::ProcessHorizontal(TEdge *horzEdge, bool isTopOfScanbeam)
{
  ...
  else if (eMaxPair)
  {
    if (eMaxPair->OutIdx >= 0)
    {
      ...
      if (eMaxPair->OutIdx >= 0)
          return; //throw clipperException("ProcessHorizontal error");
    } else
    {
...
bool Clipper::ProcessIntersections(const cInt botY, const cInt topY)
{
  ...
  //{
    ...
    //return; //throw clipperException("ProcessIntersections error");
  //}
  m_SortedEdges = 0;
  return true;
}
    ...
    while( e->NextInSEL )
    {
      ...
      if(e->Curr.X > eNext->Curr.X)
      {
        if (!IntersectPoint(*e, *eNext, Pt, m_UseFullRange) && e->Curr.X > eNext->Curr.X +1)
            return; //throw clipperException("Intersection error");
        if (Pt.Y > botY)
        {
            Pt.Y = botY;
  ...
  for (int i = 0; i < m_polyNodes.ChildCount(); i++)
  {
    ...
    else if (node.m_endtype == etClosedLine)
    {
      ...
      for (int j = len - 1; j > 0; j--)
          m_normals[j] = DoublePoint(-m_normals[(size_t)j - 1].X, -m_normals[(size_t)j - 1].Y);
...
enum struct NodeType {ntAny, ntOpen, ntClosed};

void AddPolyNodeToPolygons(const PolyNode& polynode, NodeType nodetype, Paths& paths)
{
  bool match = true;
    if (nodetype == NodeType::ntClosed)
        match = !polynode.IsOpen();
    else if (nodetype == NodeType::ntOpen)
        return;
...
void PolyTreeToPaths(const PolyTree& polytree, Paths& paths)
{
  ...
  AddPolyNodeToPolygons(polytree, NodeType::ntAny, paths);
}
//------------------------------------------------------------------------------
...
void ClosedPathsFromPolyTree(const PolyTree& polytree, Paths& paths)
{
  paths.resize(0); 
  paths.reserve(polytree.Total());
  AddPolyNodeToPolygons(polytree, NodeType::ntClosed, paths);
}
//------------------------------------------------------------------------------
