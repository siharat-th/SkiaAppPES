...
{
#if defined(_MSC_VER)
__declspec(noreturn) void statmentfatal() {}
#endif
...
bool Statement::executeStep()
{
    ...
    {
        return ret; //throw SQLite::Exception(mStmtPtr, ret);
    }
...
int Statement::exec()
{
    ...
    if (SQLITE_DONE != ret) // the statement has finished executing successfully
    {
        if (SQLITE_ROW == ret)
        {
            return ret; //throw SQLite::Exception("exec() does not expect results. Use executeStep.");
        }
        else
        {
            return ret; //throw SQLite::Exception(mStmtPtr, ret);
...
int Statement::getColumnIndex(const char* apName) const
{
    ...
    {
        //return; //throw SQLite::Exception("Unknown column name.");
#if defined(_MSC_VER)
        statmentfatal();
#else
        return 0;
    ...
    mpRefCount(NULL)
{
    ...
    if (SQLITE_OK != ret)
    {
        return; //throw SQLite::Exception(apSQLite, ret);
    }
