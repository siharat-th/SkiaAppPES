...
void Connection::connect()
{
	...
	if( !addr )
        return; //throw Wobbly( "Invalid network address" );
...
void Connection::putrequest( const char* method, const char* url )
{
	...
	if( m_State != IDLE )
        return; //throw Wobbly( "Request already issued" );
...
void Connection::putheader( const char* header, const char* value )
{
	if( m_State != REQ_STARTED )
        return; //throw Wobbly( "putheader() failed" );
...
{
	if( m_State != REQ_STARTED )
        return; //throw Wobbly( "Cannot send header" );
	m_State = IDLE;
