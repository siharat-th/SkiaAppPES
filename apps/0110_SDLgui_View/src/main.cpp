...
int main(int argc, char *argv[]) {
    ...
    topView->setBorder(0);
    topView->setLayout(GUI_LAYOUT_ABSOLUTE);
    ...
    GUI_View *view1 = GUI_View::createView( topView, "Red", 10, 20, 640, 480 );
    view1->setAlignment( GUI_ALIGN_ABSOLUTE );
    view1->setBorder( 1 );
    view1->setBackgroundColor( sdl_color( 0xFF0000FF ) );
    view1->dragable = false;
    view1->setLayout( GUI_LAYOUT_VERTICAL );

    GUI_View *view2 = GUI_View::createView( topView, "Green", 360, 380, 320, 240 );
    view2->setAlignment( GUI_ALIGN_ABSOLUTE );
    view2->setBorder( 1 );
    view2->setBackgroundColor( sdl_color( 0x00FF00FF ) );

    GUI_Label *toolbar = GUI_Label::create( view1, "Blue", 10, 20, -1, 44 );
    toolbar->setAlignment( GUI_ALIGN_TOP | GUI_ALIGN_LEFT );
    toolbar->setBorder( 1 );
    toolbar->setBackgroundColor( sdl_color( 0x0000FFFF ) );
    toolbar->dragable = false;
    toolbar->titleColor = cWhite;

    GUI_View *docView = GUI_View::createView( view1, "Blue", 10, 20, -1, -1 );
    docView->setAlignment( GUI_ALIGN_TOP | GUI_ALIGN_LEFT );
    docView->setBorder( 1 );
    docView->setBackgroundColor( sdl_color( 0xFFFFFFFF ) );
    docView->dragable = false;

    GUI_View *statusBar = GUI_View::createView( view1, "Blue", 10, 20, -1, 32 );
    statusBar->setAlignment( GUI_ALIGN_BOTTOM | GUI_ALIGN_LEFT );
    statusBar->setBorder( 1 );
    statusBar->setBackgroundColor( sdl_color( 0x8080FFFF ) );
    statusBar->dragable = false;
