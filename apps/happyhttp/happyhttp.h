...
typedef void (*ResponseData_CB)(const Response* r,
                                void* userdata,
...
public:
    ...
    Connection(const char* host,
               int port,
               int timeout = 0);  // Tooh - set send and receive timeout in seconds
    ~Connection();
...
private:
    ...
    int m_Port;
    int m_Sock;
    int m_timeout;                      // Tooh - set send and receive timeout in seconds
...
public:
    ...
    bool completed() const { return m_State == State::COMPLETE; }
