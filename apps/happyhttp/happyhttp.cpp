...
#endif
    return;  // throw Wobbly( "%s: %s", context, msg );
}
...
bool datawaiting(int sock) {
    ...
    if (r < 0) BailOnSocketError("select");
...
Wobbly::Wobbly(const char* fmt, ...) {
    ...
    va_end(ap);
    if (n == MAXLEN) m_Message[MAXLEN - 1] = '\0';
}
...
void Connection::connect() {
    ...
    if (!addr) return;  // throw Wobbly( "Invalid network address" );
    ...
    if (m_Sock < 0) BailOnSocketError("socket()");

    //	printf("Connecting to %s on port %d.\n",inet_ntoa(*addr), port);

    // Tooh - set send and receive timeout in seconds
    if (m_timeout > 0) {
#if defined(WIN32)
        DWORD timeout = (DWORD)(m_timeout * 1000);
...
#else
        struct timeval timeout = {0l};
        timeout.tv_sec = (long)m_timeout;
        timeout.tv_usec = 0l;
...
void Connection::request(const char* method,
    ...
    if (headers) {
        ...
        while (*h) {
            ...
            if (0 == _stricmp(name, "content-length")) gotcontentlength = true;
        }
    }
...
void Connection::putrequest(const char* method, const char* url) {
    // GUI_Log( "In putrequest %s\n", url );

    if (m_State != IDLE) return;  // throw Wobbly( "Request already issued" );
...
void Connection::putheader(const char* header, const char* value) {
    if (m_State != REQ_STARTED) return;  // throw Wobbly( "putheader() failed" );
...
void Connection::endheaders() {
    if (m_State != REQ_STARTED) return;  // throw Wobbly( "Cannot send header" );
    m_State = IDLE;
    ...
    for (it = m_Buffer.begin(); it != m_Buffer.end(); ++it) msg += (*it) + "\r\n";
...
char* doReplace(const char* s, char ch, const char* repl) {
    int count = 0;
    const char* t;
...
void Connection::pump() {
    ...
    if (!datawaiting(m_Sock)) return;  // recv will block

    unsigned char buf[2048];
    int a = recv(m_Sock, (char*)buf, sizeof(buf), 0);
    if (a < 0) BailOnSocketError("recv()");
    ...
    if (a == 0) {
        ...
        // assert( r->completed() ); // Jimmy - i removed this ** To examine **
        delete r;
        m_Outstanding.pop_front();
...
int Response::getstatus() const {
    ...
    assert(m_State != State::STATUSLINE);
    return m_Status;
}
...
void Response::notifyconnectionclosed() {
    ...
    // eof can be valid...
    if (m_State == State::BODY && !m_Chunked && m_Length == -1) {
        Finish();  // we're all done!
    } else {
        return;  // throw Wobbly( "Connection closed unexpectedly" );
    }
}
...
int Response::pump(const unsigned char* data, int datasize) {
    ...
    while (count > 0 && m_State != State::COMPLETE) {
        if (m_State == State::STATUSLINE || m_State == State::HEADERS ||
            m_State == State::TRAILERS || m_State == State::CHUNKLEN ||
            m_State == State::CHUNKEND) {
...
void Response::ProcessStatusLine(std::string const& line) {
    ...
    if (m_Status < 100 || m_Status > 999)
        return;  // throw Wobbly( "BadStatusLine (%s)", line.c_str() );
    ...
    */
    ...
    if (m_VersionString == "HTTP:/1.0")
        ...
        return;  // throw Wobbly( "UnknownProtocol (%s)", m_VersionString.c_str() );
...
void Response::ProcessHeaderLine(std::string const& line) {
    ...
    if (line.empty()) {
        ...
        if (m_Status == CONTINUE)
            m_State = State::STATUSLINE;  // reset parsing, expect new status line
        else
