...
//windows: tell libsdl we has main
...
//windows: tell msc we want to build app no-console when build release 
...
static void handle_events(bool *fQuit)
{
    ...
    while (SDL_PollEvent(&event))
    {
        ...
        case (SDL_DROPFILE): {  // In case if dropped file
            ...
            //SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_INFORMATION, "File or Directory dropped on window", dropped_file, window);
            app->onAppDropFile(dropped_file);
...
void sock_init(void) {
    ...
    if (0 != WSAStartup(MAKEWORD(2, 2), &wsd)) 
        SK_ABORT("Failed to initialize winsock\n");
    //if (2 != LOBYTE(wsd.wVersion) && 2 != HIBYTE(wsd.wVersion))
    //    SK_ABORT("Winsock version 2.2 is not available\n");
#endif
    sock_inited = 1;
}
