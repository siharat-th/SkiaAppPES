//
//  pesVec3f.h
//  pesEngine
//
//  Created by SIHARAT THAMMAYA on 2/7/2562 BE.
//  Copyright © 2562 SIHARAT THAMMAYA. All rights reserved.
//

#ifndef pesVec3f_h
#define pesVec3f_h

#include "pesVec2f.hpp"
#include <iostream>
#include "pesMath.hpp"


class pesVec3f {
public:
    /// \cond INTERNAL
    static const int DIM = 3;
    /// \endcond
    
    /// \brief Stores the `X` component of this vector.
    float x;
    
    /// \brief Stores the `Y` component of this vector.
    float y;
    
    /// \brief Stores the `Z` component of this vector.
    float z;
    
    //---------------------
    /// \name Construct a 3D vector
    /// \{
    
    /// \brief Construct a 3D vector. Defaults to (0,0,0).
    ///
    /// ~~~~{.cpp}
    /// pesVec3f v1;
    /// // default: v1.x is 0, v1.y is 0, v1.z is 0
    ///
    /// pesVec3f v2 = pesVec3f(40, 20, 10);
    /// // v2.x is 40, v2.y is 20, v2.z is 10
    ///
    /// pesVec3f v3(0.1, 0.3, -1.5);
    /// // v3.x is 0.1, v3.y is 0.3, v3.z is -1.5
    /// ~~~~
    pesVec3f();
    
    /// \brief Construt a 3D vector with `x`, `y` and `z` specified
    pesVec3f( float x, float y, float z=0 );
    
    /// \brief Construct a 3D vector with `x`, `y` and `z` set to `scalar`
    explicit pesVec3f( float scalar );
    
    pesVec3f( const pesVec2f& vec );
    
    pesVec3f( const pesVec3f& vec );

    //pesVec3f(const pesVec3f&) = default;
    pesVec3f(pesVec3f&&) = default;
    pesVec3f& operator=(const pesVec3f&) = default;
    pesVec3f& operator=(pesVec3f&&) = default;       
    
    /// \brief Construct a new 3D vector from a 4D vector by
    /// throwing away the 'w' component.
    ///
    /// ~~~~{.cpp}
    /// pesVec3f mom = ofVec4f(40, 20, 10, 100);
    /// pesVec3f v(mom); // v is (40, 20, 10)
    /// ~~~~
    //    pesVec3f( const ofVec4f& vec );
    
    /// \}
    
    //---------------------
    /// \name Access components
    /// \{
    
    
    /// \brief Returns a pointer to the memory position of the first element of the vector
    /// ('x'); the other elements ('y' and 'z') immediately follow it in memory.
    ///
    /// ~~~~{.cpp}
    /// pesVec3f v1 = pesVec3f(40, 20, 10);
    /// float * v1Ptr = v1.getPtr();
    /// float x = *(v1Ptr); // x is 40
    /// float y = *(v1Ptr+1); // y is 20
    /// float z = *(v1Ptr+2); // z is 10
    /// ~~~~
    ///
    /// This is very useful when using arrays of 'pesVec3f's to store geometry
    /// information, as it allows the vector to be treated as a simple C array of
    /// 'float's that can be passed verbatim to OpenGL.
    ///
    float * getPtr() {
        return (float*)&x;
    }
    const float * getPtr() const {
        return (const float *)&x;
    }
    
    
    /// \brief Allows to access the individual components of an 'pesVec3f' as though it is an
    /// array
    ///
    /// ~~~~{.cpp}
    /// pesVec3f v1 = pesVec3f(40, 20, 10);
    /// float x = v1[0]; // x is 40
    /// float y = v1[1]; // y is 20
    /// float z = v1[2]; // z is 10
    /// ~~~~
    ///
    /// This function can be handy if you want to do the same operation to all 'x',
    /// 'y' and 'z' components, as it means you can just make a 'for' loop that
    /// repeats 3 times.
    float& operator[]( int n ){
        return getPtr()[n];
    }
    
    float operator[]( int n ) const {
        return getPtr()[n];
    }
    
    
    /// \brief Set 'x', 'y' and 'z' components of this vector with just one function call.
    /// 'z' is optional, it defaults to 0.
    ///
    /// ~~~~{.cpp}
    /// pesVec3f v1;
    /// v1.set(40, 20, 70);
    /// ~~~~
    void set( float x, float y, float z = 0 );
    
    
    /// \brief Setting the values by using other 3 dimension vector pesVec3f.
    ///
    /// ~~~~{.cpp}
    /// pesVec3f v1;
    /// pesVec3f v2;
    /// v1.set(40, 20, 70);
    /// v2.set(v1);
    /// ~~~~
    void set( const pesVec3f& vec );
    void set( float _scalar );
    
    /// \}
    
    //---------------------
    /// \name Comparison
    /// \{
    
    /// \brief Check for equality between two pesVec3f
    ///
    /// Returns 'true' if each component is the same as the corresponding component in
    /// 'vec', ie if 'x == vec.x' and 'y == vec.y' and 'z == vec.z'; otherwise returns
    /// 'false'. But you should probably be using ['match'](#match) instead.
    ///
    /// ~~~~{.cpp}
    /// pesVec3f v1(40, 20, 10);
    /// pesVec3f v2(50, 30, 10);
    /// pesVec3f v3(40, 20, 10);
    /// // ( v1 == v2 ) is false
    /// // ( v1 == v3 ) is true
    /// ~~~~
    bool operator==( const pesVec3f& vec ) const;
    
    /// \brief Returns 'true' if any component is different to its corresponding component in
    /// 'vec', ie if 'x != vec.x' or 'y != vec.y' or 'z != vec.z'; otherwise returns
    /// 'false'.
    ///
    /// ~~~~{.cpp}
    /// pesVec3f v1(40, 20, 10);
    /// pesVec3f v2(50, 20, 40);
    /// pesVec3f v3(40, 20, 10);
    /// // ( v1 != v2 ) is true
    /// // ( v1 != v3 ) is false
    /// ~~~~
    bool operator!=( const pesVec3f& vec ) const;
    
    /// \brief Let you check if two vectors are similar given a tolerance threshold
    /// 'tolerance' (default = 0.0001).
    ///
    /// ~~~~{.cpp}
    /// pesVec3f v1 = pesVec3f(40, 20, 70);
    /// pesVec3f v2 = pesVec3f(40.01, 19.999, 70.05);
    /// // v1.match(v2, 0.1) is true
    /// // v1.match(v2, 0.01) is false (because (70.5-70) > 0.01)
    /// ~~~~
    ///
    bool match( const pesVec3f& vec, float tolerance = 0.0001f ) const;
    /**
     * Checks if vectors look in the same direction.
     */
    bool isAligned( const pesVec3f& vec, float tolerance = 0.0001f ) const;
    bool isAlignedRad( const pesVec3f& vec, float tolerance = 0.0001f ) const;
    
    /// \brief Returns 'true' if this vector is pointing in the same direction as
    /// 'vec', with an angle error threshold 'tolerance' in degrees (default
    /// 0.0001 degrees).
    ///
    /// ~~~~{.cpp}
    /// pesVec3f v1 = pesVec3f(40, 20, 70);
    /// pesVec3f v2 = pesVec3f(4, 2, 7);
    /// // v1.align(v2, 0.0) is true
    /// ~~~~
    bool align( const pesVec3f& vec, float tolerance = 0.0001f ) const;
    
    /// \brief Returns 'true' if this vector is pointing in the same direction
    /// as 'vec', with an angle error threshold 'tolerance' in radians
    /// (default 0.0001).
    ///
    /// ~~~~{.cpp}
    /// pesVec3f v1 = pesVec3f(40, 20, 70);
    /// pesVec3f v2 = pesVec3f(4, 2, 7);
    /// // v1.align(v2, 0.0) is true
    /// ~~~~
    bool alignRad( const pesVec3f& vec, float tolerance = 0.0001f ) const;
    
    
    /// \}
    
    //---------------------
    /// \name Operators
    /// \{
    
    /// Super easy vector addition. Returns a new vector
    /// ('x'+'vec.x','y'+'vec.y','z'+'vec.z').
    ///
    /// ~~~~{.cpp}
    /// pesVec3f v1 = pesVec3f(40, 20, 10);
    /// pesVec3f v2 = pesVec3f(25, 50, 10);
    /// pesVec3f v3 = v1 + v2; // v3 is (65, 70, 20)
    /// ~~~~
    pesVec3f  operator+( const pesVec3f& pnt ) const;
    
    /// Returns a new vector with a float value 'f' added to 'x', 'y' and 'z'
    /// members.
    ///
    /// ~~~~{.cpp}
    /// pesVec3f v1(2, 5, 1);
    /// pesVec3f v2 = v1 + 10; // (12, 15, 11)
    /// ~~~~
    
    pesVec3f  operator+( const float f ) const;
    
    /// Super easy addition assignment. Adds 'vec.x' to 'x', adds 'vec.y' to 'y' and
    /// adds 'vec.z' to 'z'.
    ///
    /// ~~~~{.cpp}
    /// pesVec3f v1 = pesVec3f(40, 20, 10);
    /// pesVec3f v2 = pesVec3f(25, 50, 10);
    /// v1 += v2; // v1 is (65, 70, 20)
    /// ~~~~
    pesVec3f& operator+=( const pesVec3f& pnt );
    
    /// Adds a float value 'f' to 'x', 'y' and 'z' members.
    ///
    /// ~~~~{.cpp}
    /// pesVec3f v1(2, 5, 1);
    /// v1 += 10; // (12, 15, 11)
    /// ~~~~
    pesVec3f& operator+=( const float f );
    
    /// Super easy vector subtraction. Returns a new vector
    /// ('x'-'vec.x','y'-'vec.y','z'-'vec.z').
    ///
    /// ~~~~{.cpp}
    /// pesVec3f v1 = pesVec3f(40, 20, 10);
    /// pesVec3f v2 = pesVec3f(25, 50, 10);
    /// pesVec3f v3 = v1 - v2; // v3 is (15, -30, 0)
    /// ~~~~
    pesVec3f  operator-( const pesVec3f& vec ) const;
    
    
    
    /// Returns a new vector with a float value 'f' subtracted from 'x', 'y' and 'z'
    /// members.
    ///
    /// ~~~~{.cpp}
    /// pesVec3f v1(2, 5, 1);
    /// pesVec3f v2 = v1 - 10; // (-8, -5, -9)
    /// ~~~~
    pesVec3f  operator-( const float f ) const;
    
    /// Returns a new 'pesVec3f' that is the inverted version (mirrored in X, Y and Z)
    /// of this vector.
    ///
    /// ~~~~{.cpp}
    /// pesVec3f v1(2, 5, 1);
    /// pesVec3f v2 = -v1; // (-2, -5, -1)
    /// ~~~~
    ///
    pesVec3f  operator-() const;
    
    /// Super easy subtraction assignment. Subtracts 'vec.x' from 'x', subtracts
    /// 'vec.y' from 'y' and subtracts 'vec.z' from 'z'.
    ///
    /// ~~~~{.cpp}
    /// pesVec3f v1 = pesVec3f(40, 20, 10);
    /// pesVec3f v2 = pesVec3f(25, 50, 10);
    /// v1 -= v2; // v1 is (15, -30, 0)
    /// ~~~~
    pesVec3f& operator-=( const pesVec3f& vec );
    
    /// Subtract a float value 'f' from 'x', 'y', and 'z' members.
    ///
    /// ~~~~{.cpp}
    /// pesVec3f v1(2, 5, 1);
    /// v1 -= 10; // (-8, -5, -9)
    /// ~~~~
    pesVec3f& operator-=( const float f );
    
    /// Returns a new vector ('x'*'vec.x','y'*'vec.y','z'*'vec.z').
    ///
    /// ~~~~{.cpp}
    /// pesVec3f v1 = pesVec3f(40, 20, 10);
    /// pesVec3f v2 = pesVec3f(2, 4, 10);
    /// pesVec3f v3 = v1 * v2; // (80, 80, 100)
    /// ~~~~
    ///
    /// Useful for scaling a 3D point by a non-uniform scale.
    ///
    pesVec3f  operator*( const pesVec3f& vec ) const;
    
    /// Return a new 'pesVec3f' that is this vector scaled by multiplying 'x', 'y', 'z'
    /// members by 'f'.
    ///
    /// ~~~~{.cpp}
    /// pesVec3f v1(2, 5, 1);
    /// pesVec3f v2 = v1 * 4; // (8, 20, 4)
    /// ~~~~
    pesVec3f  operator*( const float f ) const;
    
    /// Multiplies 'x' by 'vec.x', and multiplies 'y' by 'vec.y', and multiplies 'z'
    /// by 'vec.z'.
    ///
    /// ~~~~{.cpp}
    /// pesVec3f v1 = pesVec3f(40, 20, 10);
    /// pesVec3f v2 = pesVec3f(2, 4, 10);
    /// v1 *= v2; // v1 is now (80, 80, 100)
    /// ~~~~
    ///
    /// Useful for scaling a 3D point by a non-uniform scale.
    pesVec3f& operator*=( const pesVec3f& vec );
    
    /// Scale this vector by multiplying 'x', 'y' and 'z' members by 'f'.
    ///
    /// ~~~~{.cpp}
    /// pesVec3f v1(2, 5, 1);
    /// v1 *= 4; // (8, 20, 4)
    /// ~~~~
    pesVec3f& operator*=( const float f );
    
    /// Returns a new vector ('x'/'vec.x','y'/'vec.y','z'/'vec.z').
    ///
    /// ~~~~{.cpp}
    /// pesVec3f v1 = pesVec3f(40, 20, 10);
    /// pesVec3f v2 = pesVec3f(2, 4, 10);
    /// pesVec3f v3 = v1 / v2; // (20, 5, 1)
    /// ~~~~
    ///
    /// Useful for scaling a 3D point by a non-uniform scale.
    pesVec3f  operator/( const pesVec3f& vec ) const;
    
    /// Return a new 'pesVec3f' that is this vector scaled by dividing 'x', 'y'
    /// and 'z' members by 'f'.
    ///
    /// ~~~~{.cpp}
    /// pesVec3f v1(2, 5, 1);
    /// pesVec3f v2 = v1 / 4; // (0.5, 1.25, 0.25)
    /// ~~~~
    pesVec3f  operator/( const float f ) const;
    
    /// Divides 'x' by 'vec.x', divides 'y' by 'vec.y', and divides 'z' by 'vec.z'.
    ///
    /// ~~~~{.cpp}
    /// pesVec3f v1 = pesVec3f(40, 20, 10);
    /// pesVec3f v2 = pesVec3f(2, 4, 10);
    /// v1 *= v2; // v1 is now (20, 5, 1)
    /// ~~~~
    ///
    /// Useful for scaling a 3D point by a non-uniform scale.
    pesVec3f& operator/=( const pesVec3f& vec );
    
    /// Scale this vector by dividing 'x', 'y' and 'z' members by 'f'.
    ///
    /// ~~~~{.cpp}
    /// pesVec3f v1(2, 5, 1);
    /// v1 /= 4; // (0.5, 1.25, 0.25)
    /// ~~~~
    pesVec3f& operator/=( const float f );
    
    /// \cond INTERNAL
    friend std::ostream& operator<<(std::ostream& os, const pesVec3f& vec);
    friend std::istream& operator>>(std::istream& is, pesVec3f& vec);
    /// \endcond
    
    /// \}
    
    //---------------------
    /// \name Simple manipulations
    /// \{
    
    /// \brief Return a new 'pesVec3f' that is the result of scaling this vector up or down so that it has
    /// the requested length.
    ///
    /// ~~~~{.cpp}
    /// pesVec3f v1(3, 4); // length is 5
    /// pesVec3f v2 = v1.getScaled(15); // v2 is (9, 12), which has length of 15
    /// ~~~~
    ///
    pesVec3f  getScaled( const float length ) const;
    
    
    /// \brief Scales this vector up or down so that it has the requested length.
    ///
    /// ~~~~{.cpp}
    /// pesVec3f v1(3, 4); // length is 5
    /// v1.scale(15); // v1 is now (9, 12), which has length of 15
    /// ~~~~
    pesVec3f& scale( const float length );
    
    
    /// \brief Return a new 'pesVec3f' that is the result of rotating this vector by 'angle'
    /// degrees around the given axis.
    ///
    /// ~~~~{.cpp}
    /// pesVec3f v1(1, 0, 0);
    /// // rotate v1 around the z axis
    /// pesVec3f v2 = v1.getRotated(45, pesVec3f(0, 0, 1)); // v2 is (√2, √2, 0)
    /// // rotate v1 around the y axis
    /// pesVec3f v3 = v1.getRotated(45, pesVec3f(0, 1, 0)); // v3 is (√2, 0, √2)
    /// ~~~~
    ///
    pesVec3f  getRotated( float angle, const pesVec3f& axis ) const;
    
    /// \brief Make a copy of this vector and perform an Euler rotation of the copy around
    /// three axes: 'ax' degrees about the x axis, 'ay' about the y axis and 'az'
    /// about the z axis. Return the copy.
    ///
    /// ~~~~~{.cpp}
    /// pesVec3f v1( 1, 0, 0 );
    /// pesVec3f v2 = v1.getRotated( 0, 0, 90 ); // v is now (0, 1, 0)
    /// pesVec3f v3 = v1.getRotated( 45, 0, 0 ); // v is now (0, √2, √2)
    /// ~~~~~
    ///
    /// Watch out for gimbal lock when specifying multiple rotations in the same call.
    ///
    pesVec3f  getRotated(float ax, float ay, float az) const;
    
    /// \brief Return a new 'pesVec3f' that is the result of rotating this vector by
    /// 'angle' degrees around the axis specified by 'axis', using 'pivot' as
    /// the origin of rotation.
    pesVec3f  getRotated( float angle, const pesVec3f& pivot, const pesVec3f& axis ) const;
    
    
    /// \brief Return a new 'pesVec3f' that is the result of rotating this
    /// vector by 'angle' radians around the given axis.
    ///
    /// ~~~~{.cpp}
    /// pesVec3f v1(1, 0, 0);
    /// // rotate v1 around the z axis
    /// pesVec3f v2 = v1.getRotated(PI/4, pesVec3f(0, 0, 1)); // v2 is (√2, √2, 0)
    /// // rotate v1 around the y axis
    /// pesVec3f v3 = v1.getRotated(PI/4, pesVec3f(0, 1, 0)); // v3 is (√2, 0, √2)
    /// ~~~~
    pesVec3f  getRotatedRad( float angle, const pesVec3f& axis ) const;
    
    /// \brief Make a copy of this vector and perform an Euler rotation of the copy around
    /// three axes: 'ax' radians about the x axis, 'ay' about the y axis and 'az'
    /// about the z axis. Return the copy.
    ///
    /// ~~~~~{.cpp}
    /// pesVec3f v1( 1, 0, 0 );
    /// pesVec3f v2 = v1.getRotatedRad( 0, 0, HALF_PI ); // v is now (0, 1, 0)
    /// pesVec3f v3 = v1.getRotatedRad( PI/4, 0, 0 ); // v is now (0, √2, √2)
    /// ~~~~~
    ///
    /// Watch out for gimbal lock when specifying multiple rotations in the same call.
    ///
    pesVec3f  getRotatedRad(float ax, float ay, float az) const;
    
    /// \brief Return a new 'pesVec3f' that is the result of rotating this vector by 'angle' radians
    /// around the axis specified by 'axis', using 'pivot' as the origin of rotation.
    pesVec3f   getRotatedRad( float angle, const pesVec3f& pivot, const pesVec3f& axis ) const;
    
    /// \brief Return a new 'pesVec3f' that is the result of rotating this vector by
    /// 'angle' degrees around the given axis.
    ///
    /// ~~~~{.cpp}
    /// pesVec3f v1(1, 0, 0);
    /// // rotate v1 around the z axis
    /// v1.rotate(45, pesVec3f(0, 0, 1)); // v2 is (√2, √2, 0)
    ///
    /// v1.set(1, 0, 0);
    /// // then rotate around the y axis
    /// v1.rotate(45, pesVec3f(0, 1, 0)); // v3 is (√2, 0, √2)
    /// ~~~~
    pesVec3f& rotate( float angle, const pesVec3f& axis );
    
    /// \brief Perform an Euler rotation of this vector around three axes: 'ax' degrees about
    /// the x axis, 'ay' about the y axis and 'az' about the z axis.
    ///
    /// ~~~~~{.cpp}
    /// pesVec3f v( 1, 0, 0 );
    /// v.rotate( 0, 0, 90 ); // v is now (0, 1, 0)
    /// v.rotate( 45, 0, 0 ); // v is now (0, √2, √2)
    /// ~~~~~
    ///
    /// Watch out for gimbal lock when specifying multiple rotations in the same call.
    ///
    pesVec3f& rotate(float ax, float ay, float az);
    
    /// \brief Rotate this vector by 'angle' degrees around the axis specified by 'axis',
    /// using 'pivot' as the origin of rotation.
    pesVec3f& rotate( float angle, const pesVec3f& pivot, const pesVec3f& axis );
    
    
    /// \brief Return a new 'pesVec3f' that is the result of rotating this vector by 'angle'
    /// degrees around the given axis.
    ///
    /// ~~~~{.cpp}
    /// pesVec3f v1(1, 0, 0);
    /// // rotate v1 around the z axis
    /// v1.rotate(45, pesVec3f(0, 0, 1)); // v2 is (√2, √2, 0)
    ///
    /// v1.set(1, 0, 0);
    /// // then rotate around the y axis
    /// v1.rotate(45, pesVec3f(0, 1, 0)); // v3 is (√2, 0, √2)
    /// ~~~~
    pesVec3f& rotateRad( float angle, const pesVec3f& axis );
    
    /// \brief Perform an Euler rotation of this vector around three axes: 'ax' radians about
    /// the x axis, 'ay' about the y axis and 'az' about the z axis.
    ///
    /// ~~~~~{.cpp}
    /// pesVec3f v( 1, 0, 0 );
    /// v.rotateRad( 0, 0, HALF_PI ); // v is now (0, 1, 0)
    /// v.rotateRad( PI/4, 0, 0 ); // v is now (0, √2, √2)
    /// ~~~~~
    ///
    /// Watch out for gimbal lock when specifying multiple rotations in the same call.
    pesVec3f& rotateRad(float ax, float ay, float az);
    
    /// \brief Rotate this vector by 'angle' radians around the axis specified by 'axis',
    /// using 'pivot' as the origin of rotation.
    pesVec3f& rotateRad( float angle, const pesVec3f& pivot, const pesVec3f& axis );
    
    
    
    /// \brief Return a new 'pesVec3f' calculated by copying this vector and then mapping from
    /// its default coordinate system -- origin (0,0,0), X direction (1,0,0), Y
    /// direction (0,1,0), Z direction (0,0,1) -- to a new coordinate system defined
    /// with origin at 'origin', X direction 'vx', and Y direction 'vy', and Z
    /// direction 'vz'.
    ///
    /// *In most cases you want 'vx', 'vy', and 'vz' to be perpendicular and of unit
    /// length; if they are not perpendicular you will have shearing as part of the
    /// mapping, and if they are not of unit length you will have scaling as part of
    /// the mapping.*
    ///
    pesVec3f getMapped( const pesVec3f& origin,
                      const pesVec3f& vx,
                      const pesVec3f& vy,
                      const pesVec3f& vz ) const;
    
    /// \brief Map this vector from its default coordinate system -- origin (0,0,0), X
    /// direction (1,0,0), Y direction (0,1,0), Z direction (0,0,1) -- to a new
    /// coordinate system defined with origin at 'origin', X direction 'vx', and Y
    /// direction 'vy', and Z direction 'vz'.
    ///
    /// *In most cases you want 'vx', 'vy', and 'vz' to be perpendicular and of
    /// unit length; if they are not perpendicular you will have shearing as part
    /// of the mapping, and if they are not of unit length you will have scaling
    /// as part of the mapping.*
    ///
    pesVec3f& map( const pesVec3f& origin,
                 const pesVec3f& vx,
                 const pesVec3f& vy,
                 const pesVec3f& vz );
    
    
    
    /// \}
    
    
    //---------------------
    /// \name Distance
    /// \{
    
    
    /// \brief Treats both this vector and 'pnt' as points in 3D space, and
    /// calculates and returns the distance between them.
    ///
    /// ~~~~{.cpp}
    /// pesVec3f p1(3, 4, 2);
    /// pesVec3f p2(6, 8, 5);
    /// float distance = p1.distance( p2 ); // distance is 5.8310
    /// ~~~~
    ///
    /// 'distance' involves a square root calculation, which is one of the
    /// slowest things you can do in programming. If you don't need an exact
    /// number but rather just a rough idea of distance (for example when
    /// finding the shortest distance of a bunch of points to a reference
    /// point, where it doesn't matter exactly what the distances are, you
    /// just want the shortest), you can use squareDistance() instead.
    float distance( const pesVec3f& pnt) const;
    
    /// \brief Treats both this vector and 'pnt' as points in 3D space, and calculates and
    /// returns the squared distance between them.
    ///
    /// ~~~~{.cpp}
    /// pesVec3f p1(3, 4, 2);
    /// pesVec3f p2(6, 8, 5);
    /// float distance = p1.distance( p2 ); // distance is 5.8310
    /// ~~~~
    ///
    /// Use as a much faster alternative to distance() if you don't need
    /// to know an exact number but rather just a rough idea of distance (for example
    /// when finding the shortest distance of a bunch of points to a reference point,
    /// where it doesn't matter exactly what the distances are, you just want the
    /// shortest). It avoids the square root calculation that is ordinarily required
    /// to calculate a length.
    ///
    float squareDistance( const pesVec3f& pnt ) const;
    
    
    /// \}
    
    //---------------------
    /// \name Interpolation
    /// \{
    
    /// \brief Perform a linear interpolation of this vector's position towards 'pnt'
    /// and return the interpolated vector without altering the original. 'p'
    /// controls the amount to move towards 'pnt'. 'p' is normally between 0
    /// and 1 and where 0 means stay the original position and 1 means move
    /// all the way to 'pnt', but you can also have 'p' greater than 1
    /// overshoot 'pnt', or less than 0 to move backwards away from 'pnt'.
    ///
    /// ~~~~{.cpp}
    /// pesVec3f v1(0, 5, 0);
    /// pesVec3f v2(10, 10, 20);
    /// pesVec3f v3 = v1.getInterpolated(p2, 0.5); // v3 is (5, 7.5, 10)
    /// pesVec3f v4 = v1.getInterpolated(p2, 0.8); // v4 is (8, 9, 16)
    /// ~~~~
    ///
    pesVec3f   getInterpolated( const pesVec3f& pnt, float p ) const;
    
    /// \brief Perform a linear interpolation of this vector's position towards
    /// 'pnt'. 'p' controls the amount to move towards 'pnt'. 'p' is normally
    /// between 0 and 1 and where 0 means stay the original position and 1
    /// means move all the way to 'pnt', but you can also have 'p' greater
    /// than 1 overshoot 'pnt', or less than 0 to move backwards away from
    /// 'pnt'.
    ///
    /// ~~~~{.cpp}
    /// pesVec3f v1( 0, 5, 0 );
    /// pesVec3f v2( 10, 10, 20 );
    /// // go go gadget zeno
    /// v1.interpolate( v2, 0.5 ); // v1 is now (5, 7.5, 10)
    /// v1.interpolate( v2, 0.5 ); // v1 is now (7.5, 8.75, 15)
    /// v1.interpolate( v2, 0.5 ); // v1 is now (8.75, 9.375, 17.5)
    /// v1.interpolate( v2, 0.5 ); // v1 is now (9.375, 9.6875, 18.75)
    /// ~~~~
    pesVec3f&  interpolate( const pesVec3f& pnt, float p );
    
    /// \brief Calculate and return the midpoint between this vector and 'pnt'.
    ///
    /// ~~~~{.cpp}
    /// pesVec3f v1(5, 0, 0);
    /// pesVec3f v2(10, 10, 20);
    /// pesVec3f mid = v1.getMiddle(v2); // mid gets (7.5, 5, 10)
    /// ~~~~
    pesVec3f   getMiddle( const pesVec3f& pnt ) const;
    
    /// Set this vector to the midpoint between itself and 'pnt'.
    ///
    /// ~~~~{.cpp}
    /// pesVec3f v1( 0, 5, 0 );
    /// pesVec3f v2( 10, 10, 20);
    /// // go go gadget zeno
    /// v1.middle( v2 ); // v1 is now (5, 7.5, 10)
    /// v1.middle( v2 ); // v1 is now (7.5, 8.75, 15)
    /// v1.middle( v2 ); // v1 is now (8.75, 9.375, 17.5)
    /// v1.middle( v2 ); // v1 is now (9.375, 9.6875, 18.75)
    /// ~~~~
    pesVec3f&  middle( const pesVec3f& pnt );
    
    /// \brief Sets this vector to be the average (*centre of gravity* or
    /// *centroid*) of a given array of 'pesVec3f's. 'points' is the array of
    /// 'pesVec3f's and 'num' specifies the number of 'pesVec3f's in the array.
    ///
    /// ~~~~{.cpp}
    /// int numPoints = 10;
    /// pesVec3f points[numPoints];
    /// for ( int i=0; i<numPoints; i++ ) {
    ///     points[i].set( ofRandom(0,100), ofRandom(0,100), ofRandom(0,100) );
    /// }
    /// pesVec3f centroid;
    /// centroid.average( points, numPoints );
    /// // centroid now is the centre of gravity/average of all the random points
    /// ~~~~
    pesVec3f&  average( const pesVec3f* points, int num );
    
    /// \}
    
    //---------------------
    /// \name Limit
    /// \{
    
    /// \brief Return a normalized copy of this vector.
    ///
    /// *Normalization* means to scale the vector so that its length
    /// (magnitude) is exactly 1, at which stage all that is left is the
    /// direction. A normalized vector is usually called a *unit vector*, and
    /// can be used to represent a pure direction (heading).
    ///
    /// ~~~~{.cpp}
    /// pesVec3f v1(5, 0, 0);
    /// pesVec3f v1Normalized = v1.getNormalized(); // (1, 0, 0)
    /// pesVec3f v2(5, 0, 5);
    /// pesVec3f v2Normalized = v2.getNormalized(); // (√2, 0, √2)
    /// ~~~~
    pesVec3f  getNormalized() const;
    
    /// \brief Normalize the vector.
    ///
    /// *Normalizing* means to scale the vector so that its length (magnitude)
    /// is exactly 1, at which stage all that is left is the direction. A
    /// normalized vector is usually called a *unit vector*, and can be used
    /// to represent a pure direction (heading).
    ///
    /// ~~~~{.cpp}
    /// pesVec3f v1(5, 0, 0);
    /// v1.normalize(); // v2 is now (1, 0, 0)
    /// pesVec3f v2(5, 0, 5);
    /// v2.normalize(); // v2 is now (√2, 0, √2)
    /// ~~~~
    ///
    pesVec3f& normalize();
    
    
    /// \brief Return a copy of this vector with its length (magnitude) restricted to a
    /// maximum of 'max' units by scaling down if necessary.
    ///
    /// ~~~~{.cpp}
    /// pesVec3f v1(5, 0, 1); // length is about 5.1
    /// pesVec3f v2(2, 0, 1); // length is about 2.2
    /// pesVec3f v1Limited = v1.getLimited(3);
    /// // v1Limited is (2.9417, 0, 0.58835) which has length of 3 in the same direction as v1
    /// pesVec3f v2Limited = v2.getLimited(3);
    /// // v2Limited is (2, 0, 1) (same as v2)
    /// ~~~~
    pesVec3f  getLimited(float max) const;
    
    /// \brief Restrict the length (magnitude) of this vector to a maximum of 'max'
    /// units by scaling down if necessary.
    ///
    /// ~~~~{.cpp}
    /// pesVec3f v1(5, 0, 1); // length is about 5.1
    /// pesVec3f v2(2, 0, 1); // length is about 2.2
    /// v1.limit(3);
    /// // v1 is now (2.9417, 0, 0.58835) which has length of 3 in the same direction as at initialization
    /// v2.limit(3);
    /// // v2 is unchanged
    /// ~~~~
    pesVec3f& limit(float max);
    
    
    /// \}
    
    //---------------------
    /// \name Measurement
    /// \{
    
    
    /// Return the length (magnitude) of this vector.
    ///
    /// ~~~~{.cpp}
    /// pesVec3f v(3, 4, 1);
    /// float len = v.length(); // len is 5.0990
    /// ~~~~
    ///
    /// `length' involves a square root calculation, which is one of the
    /// slowest things you can do in programming. If you don't need an exact
    /// number but rather just a rough idea of a length (for example when
    /// finding the shortest distance of a bunch of points to a reference
    /// point, where it doesn't matter exactly what the lengths are, you just
    /// want the shortest), you can use lengthSquared() instead.
    ///
    float length() const;
    
    /// \brief Return the squared length (squared magnitude) of this vector.
    ///
    /// ~~~~{.cpp}
    /// pesVec3f v(3, 4, 1);
    /// float len = v.length(); // len is 5.0990
    /// ~~~~
    ///
    /// Use as a much faster alternative to length() if you don't need
    /// to know an accurate length but rather just a rough idea of a length (for
    /// example when finding the shortest distance of a bunch of points to a
    /// reference point, where it doesn't matter exactly what the lengths are, you
    /// just want the shortest). It avoids the square root calculation that is
    /// ordinarily required to calculate a length.
    float lengthSquared() const;
    
    /// \brief Calculate and return the coplanar angle in degrees between this vector
    /// and 'vec'.
    ///
    /// ~~~~{.cpp}
    /// pesVec3f v1(1,0,0);
    /// pesVec3f v2(0,1,0);
    /// float angle = v1.angle(v2); // angle is 90
    /// ~~~~
    float angle( const pesVec3f& vec ) const;
    
    /// \brief Calculate and return the coplanar angle in radians between this
    /// vector and 'vec'.
    ///
    /// ~~~~{.cpp}
    /// pesVec3f v1(1,0,0);
    /// pesVec3f v2(0,1,0);
    /// float angle = v1.angle(v2); // angle is 90
    /// ~~~~
    ///
    float angleRad( const pesVec3f& vec ) const;
    
    
    /// \}
    
    //---------------------
    /// \name Perpendicular
    /// \{
    
    
    /// \brief Construct a plane using this vector and 'vec' (by finding the plane that both
    /// lectors lie on), and return the vector that is perpendicular to that plane
    /// (the normal to that plane).
    ///
    /// ~~~~{.cpp}
    /// ofSetLogLevel(OF_LOG_NOTICE);
    /// pesVec3f v1(1,0,0);
    /// pesVec3f v2(0,1,0);
    /// pesVec3f p = v1.getPerpendicular(v2);
    /// ofLog(OF_LOG_NOTICE, "%1.1f, %1.1f, %1.1f\n", p.x, p.y, p.z);
    /// // prints "0.0, 0.0, 1.0"
    /// ~~~~
    ///
    /// This method is usually used to calculate a normal vector to a surface, which
    /// can then be used to calculate lighting, collisions, and other 3D effects.
    ///
    pesVec3f  getPerpendicular( const pesVec3f& vec ) const;
    
    
    /// \brief Construct a plane using this vector and 'vec' (by finding the plane that both
    /// lie on), and set our 'x', 'y' and 'z' to be the vector that is perpendicular
    /// to the constructed plane (the normal to the plane).
    ///
    /// ~~~~{.cpp}
    /// ofSetLogLevel(OF_LOG_NOTICE);
    /// pesVec3f v1(1,0,0);
    /// pesVec3f v2(0,1,0);
    /// v1.perpendicular(v2);
    /// ofLog(OF_LOG_NOTICE, "%1.1f, %1.1f, %1.1f\n", v1.x, v1.y, v1.z);
    /// // prints "0.0, 0.0, 1.0'
    /// ~~~~
    ///
    /// This method is usually used to calculate a normal vector to a surface, which
    /// can then be used to calculate lighting, collisions, and other 3D effects.
    ///
    pesVec3f& perpendicular( const pesVec3f& vec );
    
    
    
    /// \brief Returns the cross product (vector product) of this vector and 'vec'. This is a
    /// binary operation on two vectors in three-dimensional space, which results in a
    /// vector that is perpendicular to both of the vectors being multiplied, and
    /// normal to the plane containing them. The name *cross product* is derived from
    /// the cross symbol X that is often used to designate this operation; the
    /// alternative name *vector product* emphasizes the vector (rather than scalar)
    /// nature of the result.
    ///
    /// ![CROSS](math/crossproduct.png)
    /// Image courtesy of Wikipedia
    pesVec3f  getCrossed( const pesVec3f& vec ) const;
    
    /// Set this vector to the cross product (vector product) of itself and
    /// 'vec'. This is a binary operation on two vectors in three-dimensional
    /// space, which results in a vector that is perpendicular to both of the
    /// vectors being multiplied, and normal to the plane containing them. The
    /// name *cross product* is derived from the cross symbol X that is often
    /// used to designate this operation; the alternative name *vector
    /// product* emphasizes the vector (rather than scalar) nature of the
    /// result.
    pesVec3f& cross( const pesVec3f& vec );
    
    
    /// \brief Calculate and return the dot product of this vector with 'vec'.
    ///
    /// *Dot product* (less commonly known as *Euclidean inner product*)
    /// expresses the angular relationship between two vectors. In other
    /// words it is a measure of how *parallel* two vectors are. If they are
    /// completely perpendicular the dot product is 0; if they are completely
    /// parallel their dot product is either 1 if they are pointing in the
    /// same direction, or -1 if they are pointing in opposite directions.
    ///
    /// ![DOT](math/dotproduct.png)
    /// Image courtesy of Wikipedia
    ///
    /// ~~~~{.cpp}
    /// pesVec3f a1(1, 0, 0);
    /// pesVec3f b1(0, 0, 1); // 90 degree angle to a1
    /// dot = a1.dot(b1); // dot is 0, ie cos(90)
    ///
    /// pesVec3f a2(1, 0, 0);
    /// pesVec3f b2(1, 1, 0); // 45 degree angle to a2
    /// b2.normalize(); // vectors should to be unit vectors (normalized)
    /// float dot = a2.dot(b2); // dot is 0.707, ie cos(45)
    ///
    /// pesVec3f a3(0, 1, 0);
    /// pesVec3f b3(0, -1, 0); // 180 degree angle to a3
    /// dot = a3.dot(b3); // dot is -1, ie cos(180)
    /// ~~~~
    ///
    float dot( const pesVec3f& vec ) const;
    
    /// \}
    
    
    
    //    //-----------------------------------------------
    //    // this methods are deprecated in 006 please use:
    //    /// \cond INTERNAL
    //
    //    // getScaled
    //    OF_DEPRECATED_MSG("Use member method getScaled() instead.", pesVec3f rescaled( const float length ) const);
    //
    //    // scale
    //    OF_DEPRECATED_MSG("Use member method scale() instead.", pesVec3f& rescale( const float length ));
    //
    //    // getRotated
    //    OF_DEPRECATED_MSG("Use member method getRotated() instead.", pesVec3f rotated( float angle, const pesVec3f& axis ) const);
    //
    //    // getRotated should this be const???
    //    OF_DEPRECATED_MSG("Use member method getRotated() instead.", pesVec3f rotated(float ax, float ay, float az));
    //
    //    // getNormalized
    //    OF_DEPRECATED_MSG("Use member method getNormalized() instead.", pesVec3f normalized() const);
    //
    //    // getLimited
    //    OF_DEPRECATED_MSG("Use member method getLimited() instead.", pesVec3f limited(float max) const);
    //
    //    // getCrossed
    //    OF_DEPRECATED_MSG("Use member method getCrossed() instead.", pesVec3f crossed( const pesVec3f& vec ) const);
    //
    //    // getPerpendicular
    //    OF_DEPRECATED_MSG("Use member method getPerpendicular() instead.", pesVec3f perpendiculared( const pesVec3f& vec ) const);
    //
    //    // use getMapped
    //    OF_DEPRECATED_MSG("Use member method getMapped() instead.", pesVec3f mapped( const pesVec3f& origin,
    //                                                                               const pesVec3f& vx,
    //                                                                               const pesVec3f& vy,
    //                                                                               const pesVec3f& vz ) const);
    //
    //    // use squareDistance
    //    OF_DEPRECATED_MSG("Use member method squareDistance() instead.", float  distanceSquared( const pesVec3f& pnt ) const);
    //
    //    // use getInterpolated
    //    OF_DEPRECATED_MSG("Use member method getInterpolated() instead.", pesVec3f interpolated( const pesVec3f& pnt, float p ) const);
    //
    //    // use getMiddle
    //    OF_DEPRECATED_MSG("Use member method getMiddle() instead.", pesVec3f middled( const pesVec3f& pnt ) const);
    //
    //    // use getRotated
    //    OF_DEPRECATED_MSG("Use member method getRotated() instead.", pesVec3f rotated( float angle,
    //                                                                                 const pesVec3f& pivot,
    //                                                                                 const pesVec3f& axis ) const);
    
    // return all zero vector
    static pesVec3f zero() { return pesVec3f(0, 0, 0); }
    
    // return all one vector
    static pesVec3f one() { return pesVec3f(1, 1, 1); }
    
    /// \endcond
    
};


/// \cond INTERNAL


// Non-Member operators
//
//
pesVec3f operator+( float f, const pesVec3f& vec );
pesVec3f operator-( float f, const pesVec3f& vec );
pesVec3f operator*( float f, const pesVec3f& vec );
pesVec3f operator/( float f, const pesVec3f& vec );


/////////////////
// Implementation
/////////////////


inline pesVec3f::pesVec3f( const pesVec2f& vec ):x(vec.x), y(vec.y), z(0) {}
inline pesVec3f::pesVec3f( const pesVec3f& vec ):x(vec.x), y(vec.y), z(vec.z) {}
inline pesVec3f::pesVec3f(): x(0), y(0), z(0) {}
inline pesVec3f::pesVec3f( float _all ): x(_all), y(_all), z(_all) {}
inline pesVec3f::pesVec3f( float _x, float _y, float _z ):x(_x), y(_y), z(_z) {}


// Getters and Setters.
//
//
inline void pesVec3f::set( float _scalar ) {
    x = _scalar;
    y = _scalar;
    z = _scalar;
}

inline void pesVec3f::set( float _x, float _y, float _z ) {
    x = _x;
    y = _y;
    z = _z;
}

inline void pesVec3f::set( const pesVec3f& vec ) {
    x = vec.x;
    y = vec.y;
    z = vec.z;
}


// Check similarity/equality.
//
//
inline bool pesVec3f::operator==( const pesVec3f& vec ) const {
    return (x == vec.x) && (y == vec.y) && (z == vec.z);
}

inline bool pesVec3f::operator!=( const pesVec3f& vec ) const {
    return (x != vec.x) || (y != vec.y) || (z != vec.z);
}

inline bool pesVec3f::match( const pesVec3f& vec, float tolerance ) const{
    return (fabs(x - vec.x) < tolerance)
    && (fabs(y - vec.y) < tolerance)
    && (fabs(z - vec.z) < tolerance);
}

/**
 * Checks if vectors look in the same direction.
 */
inline bool pesVec3f::isAligned( const pesVec3f& vec, float tolerance ) const {
    float angle = this->angle( vec );
    return  angle < tolerance;
}
inline bool pesVec3f::align( const pesVec3f& vec, float tolerance ) const {
    return isAligned( vec, tolerance );
}

inline bool pesVec3f::isAlignedRad( const pesVec3f& vec, float tolerance ) const {
    float angle = this->angleRad( vec );
    return  angle < tolerance;
}
inline bool pesVec3f::alignRad( const pesVec3f& vec, float tolerance ) const {
    return isAlignedRad( vec, tolerance );
}


// Operator overloading for pesVec3f
//
//

inline std::ostream& operator<<(std::ostream& os, const pesVec3f& vec) {
    os << vec.x << ", " << vec.y << ", " << vec.z;
    return os;
}

inline std::istream& operator>>(std::istream& is, pesVec3f& vec) {
    is >> vec.x;
    is.ignore(2);
    is >> vec.y;
    is.ignore(2);
    is >> vec.z;
    return is;
}

inline pesVec3f pesVec3f::operator+( const pesVec3f& pnt ) const {
    return pesVec3f( x+pnt.x, y+pnt.y, z+pnt.z );
}

inline pesVec3f& pesVec3f::operator+=( const pesVec3f& pnt ) {
    x+=pnt.x;
    y+=pnt.y;
    z+=pnt.z;
    return *this;
}

inline pesVec3f pesVec3f::operator-( const pesVec3f& vec ) const {
    return pesVec3f( x-vec.x, y-vec.y, z-vec.z );
}

inline pesVec3f& pesVec3f::operator-=( const pesVec3f& vec ) {
    x -= vec.x;
    y -= vec.y;
    z -= vec.z;
    return *this;
}

inline pesVec3f pesVec3f::operator*( const pesVec3f& vec ) const {
    return pesVec3f( x*vec.x, y*vec.y, z*vec.z );
}

inline pesVec3f& pesVec3f::operator*=( const pesVec3f& vec ) {
    x*=vec.x;
    y*=vec.y;
    z*=vec.z;
    return *this;
}

inline pesVec3f pesVec3f::operator/( const pesVec3f& vec ) const {
    return pesVec3f( vec.x!=0 ? x/vec.x : x , vec.y!=0 ? y/vec.y : y, vec.z!=0 ? z/vec.z : z );
}

inline pesVec3f& pesVec3f::operator/=( const pesVec3f& vec ) {
    vec.x!=0 ? x/=vec.x : x;
    vec.y!=0 ? y/=vec.y : y;
    vec.z!=0 ? z/=vec.z : z;
    return *this;
}

inline pesVec3f pesVec3f::operator-() const {
    return pesVec3f( -x, -y, -z );
}


//operator overloading for float
//
//
//inline void pesVec3f::operator=( const float f){
//    x = f;
//    y = f;
//    z = f;
//}

inline pesVec3f pesVec3f::operator+( const float f ) const {
    return pesVec3f( x+f, y+f, z+f);
}

inline pesVec3f& pesVec3f::operator+=( const float f ) {
    x += f;
    y += f;
    z += f;
    return *this;
}

inline pesVec3f pesVec3f::operator-( const float f ) const {
    return pesVec3f( x-f, y-f, z-f);
}

inline pesVec3f& pesVec3f::operator-=( const float f ) {
    x -= f;
    y -= f;
    z -= f;
    return *this;
}

inline pesVec3f pesVec3f::operator*( const float f ) const {
    return pesVec3f( x*f, y*f, z*f );
}

inline pesVec3f& pesVec3f::operator*=( const float f ) {
    x*=f;
    y*=f;
    z*=f;
    return *this;
}

inline pesVec3f pesVec3f::operator/( const float f ) const {
    if(f == 0) return pesVec3f( x, y, z);
    
    return pesVec3f( x/f, y/f, z/f );
}

inline pesVec3f& pesVec3f::operator/=( const float f ) {
    if(f == 0) return *this;
    
    x/=f;
    y/=f;
    z/=f;
    return *this;
}


//Scale
//
//
//inline pesVec3f pesVec3f::rescaled( const float length ) const {
//    return getScaled(length);
//}

inline pesVec3f pesVec3f::getScaled( const float length ) const {
    float l = (float)sqrt(x*x + y*y + z*z);
    if( l > 0 )
        return pesVec3f( (x/l)*length, (y/l)*length, (z/l)*length );
    else
        return pesVec3f();
}
//inline pesVec3f& pesVec3f::rescale( const float length ) {
//    return scale(length);
//}
inline pesVec3f& pesVec3f::scale( const float length ) {
    float l = (float)sqrt(x*x + y*y + z*z);
    if (l > 0) {
        x = (x/l)*length;
        y = (y/l)*length;
        z = (z/l)*length;
    }
    return *this;
}



// Rotation
//
//
//inline pesVec3f pesVec3f::rotated( float angle, const pesVec3f& axis ) const {
//    return getRotated(angle, axis);
//}
inline pesVec3f pesVec3f::getRotated( float angle, const pesVec3f& axis ) const {
    pesVec3f ax = axis.getNormalized();
    float a = (float)(angle*DEG_TO_RAD);
    float sina = sin( a );
    float cosa = cos( a );
    float cosb = 1.0f - cosa;
    
    return pesVec3f( x*(ax.x*ax.x*cosb + cosa)
                   + y*(ax.x*ax.y*cosb - ax.z*sina)
                   + z*(ax.x*ax.z*cosb + ax.y*sina),
                   x*(ax.y*ax.x*cosb + ax.z*sina)
                   + y*(ax.y*ax.y*cosb + cosa)
                   + z*(ax.y*ax.z*cosb - ax.x*sina),
                   x*(ax.z*ax.x*cosb - ax.y*sina)
                   + y*(ax.z*ax.y*cosb + ax.x*sina)
                   + z*(ax.z*ax.z*cosb + cosa) );
}

inline pesVec3f pesVec3f::getRotatedRad( float angle, const pesVec3f& axis ) const {
    pesVec3f ax = axis.getNormalized();
    float a = angle;
    float sina = sin( a );
    float cosa = cos( a );
    float cosb = 1.0f - cosa;
    
    return pesVec3f( x*(ax.x*ax.x*cosb + cosa)
                   + y*(ax.x*ax.y*cosb - ax.z*sina)
                   + z*(ax.x*ax.z*cosb + ax.y*sina),
                   x*(ax.y*ax.x*cosb + ax.z*sina)
                   + y*(ax.y*ax.y*cosb + cosa)
                   + z*(ax.y*ax.z*cosb - ax.x*sina),
                   x*(ax.z*ax.x*cosb - ax.y*sina)
                   + y*(ax.z*ax.y*cosb + ax.x*sina)
                   + z*(ax.z*ax.z*cosb + cosa) );
}

inline pesVec3f& pesVec3f::rotate( float angle, const pesVec3f& axis ) {
    pesVec3f ax = axis.getNormalized();
    float a = (float)(angle*DEG_TO_RAD);
    float sina = sin( a );
    float cosa = cos( a );
    float cosb = 1.0f - cosa;
    
    float nx = x*(ax.x*ax.x*cosb + cosa)
    + y*(ax.x*ax.y*cosb - ax.z*sina)
    + z*(ax.x*ax.z*cosb + ax.y*sina);
    float ny = x*(ax.y*ax.x*cosb + ax.z*sina)
    + y*(ax.y*ax.y*cosb + cosa)
    + z*(ax.y*ax.z*cosb - ax.x*sina);
    float nz = x*(ax.z*ax.x*cosb - ax.y*sina)
    + y*(ax.z*ax.y*cosb + ax.x*sina)
    + z*(ax.z*ax.z*cosb + cosa);
    x = nx; y = ny; z = nz;
    return *this;
}


inline pesVec3f& pesVec3f::rotateRad(float angle, const pesVec3f& axis ) {
    pesVec3f ax = axis.getNormalized();
    float a = angle;
    float sina = sin( a );
    float cosa = cos( a );
    float cosb = 1.0f - cosa;
    
    float nx = x*(ax.x*ax.x*cosb + cosa)
    + y*(ax.x*ax.y*cosb - ax.z*sina)
    + z*(ax.x*ax.z*cosb + ax.y*sina);
    float ny = x*(ax.y*ax.x*cosb + ax.z*sina)
    + y*(ax.y*ax.y*cosb + cosa)
    + z*(ax.y*ax.z*cosb - ax.x*sina);
    float nz = x*(ax.z*ax.x*cosb - ax.y*sina)
    + y*(ax.z*ax.y*cosb + ax.x*sina)
    + z*(ax.z*ax.z*cosb + cosa);
    x = nx; y = ny; z = nz;
    return *this;
}

// const???
//inline pesVec3f pesVec3f::rotated(float ax, float ay, float az) {
//    return getRotated(ax,ay,az);
//}

inline pesVec3f pesVec3f::getRotated(float ax, float ay, float az) const {
    float a = (float)cos(DEG_TO_RAD*(ax));
    float b = (float)sin(DEG_TO_RAD*(ax));
    float c = (float)cos(DEG_TO_RAD*(ay));
    float d = (float)sin(DEG_TO_RAD*(ay));
    float e = (float)cos(DEG_TO_RAD*(az));
    float f = (float)sin(DEG_TO_RAD*(az));
    
    float nx = c * e * x - c * f * y + d * z;
    float ny = (a * f + b * d * e) * x + (a * e - b * d * f) * y - b * c * z;
    float nz = (b * f - a * d * e) * x + (a * d * f + b * e) * y + a * c * z;
    
    return pesVec3f( nx, ny, nz );
}

inline pesVec3f pesVec3f::getRotatedRad(float ax, float ay, float az) const {
    float a = cos(ax);
    float b = sin(ax);
    float c = cos(ay);
    float d = sin(ay);
    float e = cos(az);
    float f = sin(az);
    
    float nx = c * e * x - c * f * y + d * z;
    float ny = (a * f + b * d * e) * x + (a * e - b * d * f) * y - b * c * z;
    float nz = (b * f - a * d * e) * x + (a * d * f + b * e) * y + a * c * z;
    
    return pesVec3f( nx, ny, nz );
}


inline pesVec3f& pesVec3f::rotate(float ax, float ay, float az) {
    float a = (float)cos(DEG_TO_RAD*(ax));
    float b = (float)sin(DEG_TO_RAD*(ax));
    float c = (float)cos(DEG_TO_RAD*(ay));
    float d = (float)sin(DEG_TO_RAD*(ay));
    float e = (float)cos(DEG_TO_RAD*(az));
    float f = (float)sin(DEG_TO_RAD*(az));
    
    float nx = c * e * x - c * f * y + d * z;
    float ny = (a * f + b * d * e) * x + (a * e - b * d * f) * y - b * c * z;
    float nz = (b * f - a * d * e) * x + (a * d * f + b * e) * y + a * c * z;
    
    x = nx; y = ny; z = nz;
    return *this;
}


inline pesVec3f& pesVec3f::rotateRad(float ax, float ay, float az) {
    float a = cos(ax);
    float b = sin(ax);
    float c = cos(ay);
    float d = sin(ay);
    float e = cos(az);
    float f = sin(az);
    
    float nx = c * e * x - c * f * y + d * z;
    float ny = (a * f + b * d * e) * x + (a * e - b * d * f) * y - b * c * z;
    float nz = (b * f - a * d * e) * x + (a * d * f + b * e) * y + a * c * z;
    
    x = nx; y = ny; z = nz;
    return *this;
}


// Rotate point by angle (deg) around line defined by pivot and axis.
//
//
//inline pesVec3f pesVec3f::rotated( float angle,
//                                const pesVec3f& pivot,
//                                const pesVec3f& axis ) const{
//    return getRotated(angle, pivot, axis);
//}

inline pesVec3f pesVec3f::getRotated( float angle,
                                   const pesVec3f& pivot,
                                   const pesVec3f& axis ) const
{
    pesVec3f ax = axis.getNormalized();
    float tx = x - pivot.x;
    float ty = y - pivot.y;
    float tz = z - pivot.z;
    
    float a = (float)(angle*DEG_TO_RAD);
    float sina = sin( a );
    float cosa = cos( a );
    float cosb = 1.0f - cosa;
    
    float xrot = tx*(ax.x*ax.x*cosb + cosa)
    + ty*(ax.x*ax.y*cosb - ax.z*sina)
    + tz*(ax.x*ax.z*cosb + ax.y*sina);
    float yrot = tx*(ax.y*ax.x*cosb + ax.z*sina)
    + ty*(ax.y*ax.y*cosb + cosa)
    + tz*(ax.y*ax.z*cosb - ax.x*sina);
    float zrot = tx*(ax.z*ax.x*cosb - ax.y*sina)
    + ty*(ax.z*ax.y*cosb + ax.x*sina)
    + tz*(ax.z*ax.z*cosb + cosa);
    
    
    return pesVec3f( xrot+pivot.x, yrot+pivot.y, zrot+pivot.z );
}


inline pesVec3f pesVec3f::getRotatedRad( float angle,
                                      const pesVec3f& pivot,
                                      const pesVec3f& axis ) const
{
    pesVec3f ax = axis.getNormalized();
    float tx = x - pivot.x;
    float ty = y - pivot.y;
    float tz = z - pivot.z;
    
    float a = angle;
    float sina = sin( a );
    float cosa = cos( a );
    float cosb = 1.0f - cosa;
    
    float xrot = tx*(ax.x*ax.x*cosb + cosa)
    + ty*(ax.x*ax.y*cosb - ax.z*sina)
    + tz*(ax.x*ax.z*cosb + ax.y*sina);
    float yrot = tx*(ax.y*ax.x*cosb + ax.z*sina)
    + ty*(ax.y*ax.y*cosb + cosa)
    + tz*(ax.y*ax.z*cosb - ax.x*sina);
    float zrot = tx*(ax.z*ax.x*cosb - ax.y*sina)
    + ty*(ax.z*ax.y*cosb + ax.x*sina)
    + tz*(ax.z*ax.z*cosb + cosa);
    
    
    return pesVec3f( xrot+pivot.x, yrot+pivot.y, zrot+pivot.z );
}


inline pesVec3f& pesVec3f::rotate( float angle,
                                const pesVec3f& pivot,
                                const pesVec3f& axis )
{
    pesVec3f ax = axis.getNormalized();
    x -= pivot.x;
    y -= pivot.y;
    z -= pivot.z;
    
    float a = (float)(angle*DEG_TO_RAD);
    float sina = sin( a );
    float cosa = cos( a );
    float cosb = 1.0f - cosa;
    
    float xrot = x*(ax.x*ax.x*cosb + cosa)
    + y*(ax.x*ax.y*cosb - ax.z*sina)
    + z*(ax.x*ax.z*cosb + ax.y*sina);
    float yrot = x*(ax.y*ax.x*cosb + ax.z*sina)
    + y*(ax.y*ax.y*cosb + cosa)
    + z*(ax.y*ax.z*cosb - ax.x*sina);
    float zrot = x*(ax.z*ax.x*cosb - ax.y*sina)
    + y*(ax.z*ax.y*cosb + ax.x*sina)
    + z*(ax.z*ax.z*cosb + cosa);
    
    x = xrot + pivot.x;
    y = yrot + pivot.y;
    z = zrot + pivot.z;
    
    return *this;
}


inline pesVec3f& pesVec3f::rotateRad( float angle,
                                   const pesVec3f& pivot,
                                   const pesVec3f& axis )
{
    pesVec3f ax = axis.getNormalized();
    x -= pivot.x;
    y -= pivot.y;
    z -= pivot.z;
    
    float a = angle;
    float sina = sin( a );
    float cosa = cos( a );
    float cosb = 1.0f - cosa;
    
    float xrot = x*(ax.x*ax.x*cosb + cosa)
    + y*(ax.x*ax.y*cosb - ax.z*sina)
    + z*(ax.x*ax.z*cosb + ax.y*sina);
    float yrot = x*(ax.y*ax.x*cosb + ax.z*sina)
    + y*(ax.y*ax.y*cosb + cosa)
    + z*(ax.y*ax.z*cosb - ax.x*sina);
    float zrot = x*(ax.z*ax.x*cosb - ax.y*sina)
    + y*(ax.z*ax.y*cosb + ax.x*sina)
    + z*(ax.z*ax.z*cosb + cosa);
    
    x = xrot + pivot.x;
    y = yrot + pivot.y;
    z = zrot + pivot.z;
    
    return *this;
}




// Map point to coordinate system defined by origin, vx, vy, and vz.
//
//
//inline pesVec3f pesVec3f::mapped( const pesVec3f& origin,
//                               const pesVec3f& vx,
//                               const pesVec3f& vy,
//                               const pesVec3f& vz ) const{
//    return getMapped(origin, vx, vy, vz);
//}

inline pesVec3f pesVec3f::getMapped( const pesVec3f& origin,
                                  const pesVec3f& vx,
                                  const pesVec3f& vy,
                                  const pesVec3f& vz ) const
{
    return pesVec3f( origin.x + x*vx.x + y*vy.x + z*vz.x,
                   origin.y + x*vx.y + y*vy.y + z*vz.y,
                   origin.z + x*vx.z + y*vy.z + z*vz.z );
}

inline pesVec3f& pesVec3f::map( const pesVec3f& origin,
                             const pesVec3f& vx,
                             const pesVec3f& vy,
                             const pesVec3f& vz )
{
    float xmap = origin.x + x*vx.x + y*vy.x + z*vz.x;
    float ymap =  origin.y + x*vx.y + y*vy.y + z*vz.y;
    z = origin.z + x*vx.z + y*vy.z + z*vz.z;
    x = xmap;
    y = ymap;
    return *this;
}


// Distance between two points.
//
//
inline float pesVec3f::distance( const pesVec3f& pnt) const {
    float vx = x-pnt.x;
    float vy = y-pnt.y;
    float vz = z-pnt.z;
    return (float)sqrt(vx*vx + vy*vy + vz*vz);
}

//inline float  pesVec3f::distanceSquared( const pesVec3f& pnt ) const{
//    return squareDistance(pnt);
//}

inline float  pesVec3f::squareDistance( const pesVec3f& pnt ) const {
    float vx = x-pnt.x;
    float vy = y-pnt.y;
    float vz = z-pnt.z;
    return vx*vx + vy*vy + vz*vz;
}



// Linear interpolation.
//
//
/**
 * p==0.0 results in this point, p==0.5 results in the
 * midpoint, and p==1.0 results in pnt being returned.
 */

//inline pesVec3f pesVec3f::interpolated( const pesVec3f& pnt, float p ) const {
//    return getInterpolated(pnt,p);
//}

inline pesVec3f pesVec3f::getInterpolated( const pesVec3f& pnt, float p ) const {
    return pesVec3f( x*(1-p) + pnt.x*p,
                   y*(1-p) + pnt.y*p,
                   z*(1-p) + pnt.z*p );
}

inline pesVec3f& pesVec3f::interpolate( const pesVec3f& pnt, float p ) {
    x = x*(1-p) + pnt.x*p;
    y = y*(1-p) + pnt.y*p;
    z = z*(1-p) + pnt.z*p;
    return *this;
}


//inline pesVec3f pesVec3f::middled( const pesVec3f& pnt ) const {
//    return getMiddle(pnt);
//}

inline pesVec3f pesVec3f::getMiddle( const pesVec3f& pnt ) const {
    return pesVec3f( (x+pnt.x)/2.0f, (y+pnt.y)/2.0f, (z+pnt.z)/2.0f );
}

inline pesVec3f& pesVec3f::middle( const pesVec3f& pnt ) {
    x = (x+pnt.x)/2.0f;
    y = (y+pnt.y)/2.0f;
    z = (z+pnt.z)/2.0f;
    return *this;
}


// Average (centroid) among points.
// Addition is sometimes useful for calculating averages too.
//
//
inline pesVec3f& pesVec3f::average( const pesVec3f* points, int num ) {
    x = 0.f;
    y = 0.f;
    z = 0.f;
    for( int i=0; i<num; i++) {
        x += points[i].x;
        y += points[i].y;
        z += points[i].z;
    }
    x /= num;
    y /= num;
    z /= num;
    return *this;
}



// Normalization
//
//
//inline pesVec3f pesVec3f::normalized() const {
//    return getNormalized();
//}

inline pesVec3f pesVec3f::getNormalized() const {
    float length = (float)sqrt(x*x + y*y + z*z);
    if( length > 0 ) {
        return pesVec3f( x/length, y/length, z/length );
    } else {
        return pesVec3f();
    }
}

inline pesVec3f& pesVec3f::normalize() {
    float length = (float)sqrt(x*x + y*y + z*z);
    if( length > 0 ) {
        x /= length;
        y /= length;
        z /= length;
    }
    return *this;
}



// Limit length.
//
//

//inline pesVec3f pesVec3f::limited(float max) const {
//    return getLimited(max);
//}

inline pesVec3f pesVec3f::getLimited(float max) const {
    pesVec3f limited;
    float lengthSquared = (x*x + y*y + z*z);
    if( lengthSquared > max*max && lengthSquared > 0 ) {
        float ratio = max/(float)sqrt(lengthSquared);
        limited.set( x*ratio, y*ratio, z*ratio);
    } else {
        limited.set(x,y,z);
    }
    return limited;
}

inline pesVec3f& pesVec3f::limit(float max) {
    float lengthSquared = (x*x + y*y + z*z);
    if( lengthSquared > max*max && lengthSquared > 0 ) {
        float ratio = max/(float)sqrt(lengthSquared);
        x *= ratio;
        y *= ratio;
        z *= ratio;
    }
    return *this;
}


// Perpendicular vector.
//
//
//inline pesVec3f pesVec3f::crossed( const pesVec3f& vec ) const {
//    return getCrossed(vec);
//}
inline pesVec3f pesVec3f::getCrossed( const pesVec3f& vec ) const {
    return pesVec3f( y*vec.z - z*vec.y,
                   z*vec.x - x*vec.z,
                   x*vec.y - y*vec.x );
}

inline pesVec3f& pesVec3f::cross( const pesVec3f& vec ) {
    float _x = y*vec.z - z*vec.y;
    float _y = z*vec.x - x*vec.z;
    z = x*vec.y - y*vec.x;
    x = _x;
    y = _y;
    return *this;
}

/**
 * Normalized perpendicular.
 */
//inline pesVec3f pesVec3f::perpendiculared( const pesVec3f& vec ) const {
//    return getPerpendicular(vec);
//}

inline pesVec3f pesVec3f::getPerpendicular( const pesVec3f& vec ) const {
    float crossX = y*vec.z - z*vec.y;
    float crossY = z*vec.x - x*vec.z;
    float crossZ = x*vec.y - y*vec.x;
    
    float length = (float)sqrt(crossX*crossX +
                               crossY*crossY +
                               crossZ*crossZ);
    
    if( length > 0 )
        return pesVec3f( crossX/length, crossY/length, crossZ/length );
    else
        return pesVec3f();
}

inline pesVec3f& pesVec3f::perpendicular( const pesVec3f& vec ) {
    float crossX = y*vec.z - z*vec.y;
    float crossY = z*vec.x - x*vec.z;
    float crossZ = x*vec.y - y*vec.x;
    
    float length = (float)sqrt(crossX*crossX +
                               crossY*crossY +
                               crossZ*crossZ);
    
    if( length > 0 ) {
        x = crossX/length;
        y = crossY/length;
        z = crossZ/length;
    } else {
        x = 0.f;
        y = 0.f;
        z = 0.f;
    }
    
    return *this;
}


// Length
//
//
inline float pesVec3f::length() const {
    return (float)sqrt( x*x + y*y + z*z );
}

inline float pesVec3f::lengthSquared() const {
    return (float)(x*x + y*y + z*z);
}



/**
 * Angle (deg) between two vectors.
 * This is an unsigned relative angle from 0 to 180.
 * http://www.euclideanspace.com/maths/algebra/vectors/angleBetween/index.htm
 */
inline float pesVec3f::angle( const pesVec3f& vec ) const {
    pesVec3f n1 = this->getNormalized();
    pesVec3f n2 = vec.getNormalized();
    return (float)(acos( n1.dot(n2) )*RAD_TO_DEG);
}

inline float pesVec3f::angleRad( const pesVec3f& vec ) const {
    pesVec3f n1 = this->getNormalized();
    pesVec3f n2 = vec.getNormalized();
    return (float)acos( n1.dot(n2) );
}



/**
 * Dot Product.
 */
inline float pesVec3f::dot( const pesVec3f& vec ) const {
    return x*vec.x + y*vec.y + z*vec.z;
}





// Non-Member operators
//
//
inline pesVec3f operator+( float f, const pesVec3f& vec ) {
    return pesVec3f( f+vec.x, f+vec.y, f+vec.z );
}

inline pesVec3f operator-( float f, const pesVec3f& vec ) {
    return pesVec3f( f-vec.x, f-vec.y, f-vec.z );
}

inline pesVec3f operator*( float f, const pesVec3f& vec ) {
    return pesVec3f( f*vec.x, f*vec.y, f*vec.z );
}

inline pesVec3f operator/( float f, const pesVec3f& vec ) {
    return pesVec3f( f/vec.x, f/vec.y, f/vec.z);
}

/// \endcond

#endif /* pesVec3f_h */
