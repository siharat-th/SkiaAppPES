...
private:
    ...
    inline void check(const int aRet) const
    {
        if (SQLite::OK != aRet)
        {
            return; //throw SQLite::Exception(mStmtPtr, aRet);
        }
    }
    ...
    inline void checkRow() const
    {
        if (false == mbHasRow)
        {
            return; //throw SQLite::Exception("No row to get a column from. executeStep() was not called, or returned false.");
        }
    }
    ...
    inline void checkIndex(const int aIndex) const
    {
        ...
        {
            return; //throw SQLite::Exception("Column index out of range.");
        }
    }
