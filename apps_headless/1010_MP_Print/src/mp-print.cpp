...
int main( int argc, char *argv[] ) {
    ...
    std::ifstream f("mp2_pid2sku.json");
    json pid2sku = json::parse(f);

    std::string s = pid2sku[pid];
    printf( "SKU: %s\n", s.c_str() );

    /*
    SVG *svg = loadSVG( "./tiger.svg" );
    printf( "SVG: %0.2f %0.2f, %0.2f %0.2f\n", 
        svg->width, svg->height,
        svg->width_72dpi, svg->height_72dpi );

    PDF *pdf = createPDF();
    SkCanvas* canvas = pdf->pdfDoc->beginPage(svg->width_72dpi, svg->height_72dpi);

    canvas->save();
    canvas->scale( svg->width_72dpi / svg->width, svg->height_72dpi / svg->height );
    svg->svgDom->render( canvas );
    canvas->restore();
    
    pdf->pdfDoc->endPage();
    SkData *data = closePDF( pdf );
    SkFILEWStream out("svg-2-pdf.pdf");
