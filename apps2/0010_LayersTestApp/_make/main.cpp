...
int main(int, char **)
{
    ...
    const char title[] = "LayersTestSdlApp";
...
// - opengles2
...
#endif
    SDL_Renderer* GUI_renderer = SDL_CreateRenderer(window, -1, flags);
    if (GUI_renderer == NULL) {
        printf("GUI_CreateRenderer Error");
    ...
    for (Uint32 i = 0; i < rendererinfo.num_texture_formats; i++) {
        std::cout << SDL_GetPixelFormatName(rendererinfo.texture_formats[i]) << std::endl;
    }
    std::cout << std::endl;
