...
bool DecSVGDomEditor::onMouse(int x, int y, skui::InputState state, skui::ModifierKey mod) {
    ...
    if (state == skui::InputState::kDown) {
        bDragTest = true;
            ...
            if (_bEnableMouseHoverMode && pHoverNode && !bMove) {
                pNode = pHoverNode;
        } else {
            pNode = selectNodeByPosition(x, y, _svgViewScale, pSelectedNode);
            }
            if (pNode) {
    ...
    if (bMove) {
        if (_bEnableMouseHoverMode) {
            SkSVGNode* pNode = selectNodeByPosition(x, y, _svgViewScale);
            if (pNode) {
                if (pHoverNode != pNode) {
                    dirty(true);
                }
...
SkSVGNode* DecSVGDomEditor::selectNodeByPosition(float x, float y, float scale, SkSVGNode* pSelectedNode) {
    SkSVGNode* pNode = NULL;
    if (fDom) {
