...
void CieLab::init()
{
    if (!_clab_inited_)
    {
        cbrt_table[0] = (float)pow(float(1)/float(ROOT_TAB_SIZE*2), 0.3333);
        qn_table[0]   = (float)pow(float(1)/float(ROOT_TAB_SIZE*2), 0.2);
        for(int i = 1; i < ROOT_TAB_SIZE +1; i++)
        {
            cbrt_table[i] = (float)pow(float(i)/float(ROOT_TAB_SIZE), 0.3333);
            qn_table[i] = (float)pow(float(i)/float(ROOT_TAB_SIZE), 0.2);
        }
        _clab_inited_ = true;
    }
}
...
//        GUI_LogWarning("getConfidence", "out of bounds (%d,%d)/(%d,%d)", x, y, width, height);
                                  ...
                                  unsigned int backgroundFillColor)
{
    ...
    for (unsigned int i=0; i<pixelCount; i++)
    {
        ...
        {
            float progress =
            30.0f + 60.0f * (float)i / (float)pixelCount;
                                ...
                                const unsigned int dims)
{
    ...
    {
        float pivotPoint = (min + max) / 2.0f; //average
    ...
    else
    {
        ...
        if (newpoint.C != 0)
            newpoint.mul(1.0f / (float)newpoint.C);
                          ...
                          const unsigned int dims)
{
    ...
    colorSignatureStage2(input, 0, stage1length, 0, &stage2length, length * 0.001f, dims);
