...
void DecSVGEditor::appendSewingLine() {
    ...
    clearEditingSewingLine();
    pNodeTree->select_treeItem(NULL);
...
void DecSVGEditor::rotateSewingLine() {
    ...
    for( int i=0; i<5; i++ ) {
        ...
        float s = sin( 0 - 3.14159265 / 2 ); 
        float c = cos( 0 - 3.14159265 / 2 ); 
        ...
        if( std::filesystem::exists( fn ) ) {
            ...
            for( int j=0; j<_vSewingLineInfo[i].size(); j++ ) {
                ...
                if( !strcmp( verb, "M" ) ) {
                    path->moveTo( atof(x) * c - atof(y) * s, atof(x) * s + atof(y) * c + h );
                }
                else if( !strcmp( verb, "L" ) ) {
                    path->lineTo( atof(x) * c - atof(y) * s, atof(x) * s + atof(y) * c + h );
                }
                else if( !strcmp( verb, "C" ) ) {
                    char *x2 = strtok (NULL, " ");
                    char *y2 = strtok (NULL, " ");
                    char *x3 = strtok (NULL, " ");
                    char *y3 = strtok (NULL, " ");
                    path->cubicTo( atof(x) * c - atof(y) * s, atof(x) * s + atof(y) * c + h, 
                        atof(x2) * c - atof(y2) * s, atof(x2) * s + atof(y2) * c + h, 
                        atof(x3) * c - atof(y3) * s, atof(x3) * s + atof(y3) * c + h );
                }
                else if( !strcmp( verb, "Q" ) ) {
                    char *x2 = strtok (NULL, " ");
                    char *y2 = strtok (NULL, " ");
                    path->quadTo( atof(x) * c - atof(y) * s, atof(x) * s + atof(y) * c + h, 
                        atof(x2) * c - atof(y2) * s, atof(x2) * s + atof(y2) * c + h );
                }
                else if( !strcmp( verb, "N" ) ) {
                    char *x2 = strtok (NULL, " ");
...
void DecSVGEditor::drawSewingLine( DecSVGDomEditor *, SkCanvas &canvas ) {
    ...
    if( _sewingLinePath[size_index] || _editingSewingLine) {
        ...
        paint.setStrokeWidth( 2/pSVGDomEditor->drawScale_DPI );
