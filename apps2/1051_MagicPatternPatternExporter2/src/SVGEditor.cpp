...
const char *pocketFileList[] = {
    "ฝากระเป๋า(Plus-Size)-4แบบ-A3=1-01.svg",
    "กระเป๋าปะ(Plus-Size)-จีบBox-จีบคว่ำ-ทรงโค้ง-A3=1-01.svg",
    "กระเป๋าปะ(Plus-Size)-จีบBox-จีบคว่ำ-ทรงหกเหลี่ยม-A3=1-01.svg",
    "กระเป๋าปะ(Plus-Size)-จีบBox-จีบคว่ำ-ทรงสี่เหลี่ยม-A3=1-01.svg",
    "กระเป๋าปะ(Plus-Size)-ทรงแหลม-A3=1-01.svg",
    "กระเป๋าปะ(Plus-Size)-ทรงโค้ง-A3=1-01.svg",
    "กระเป๋าปะ(Plus-Size)-ทรงหกเหลี่ยม-A3=1-01.svg",
    "กระเป๋าปะ(Plus-Size)-ทรงสี่เหลี่ยม-A3=1-01.svg",
    "ฝากระเป๋า-4แบบ-A4=2-01.svg",
    "ฝากระเป๋า-4แบบ-A4=2-02.svg",
    "กระเป๋าปะ-จีบBox-จีบคว่ำ-ทรงโค้ง-A4=1-01.svg",
    "กระเป๋าปะ-จีบBox-จีบคว่ำ-ทรงหกเหลี่ยม-A4=1-01.svg",
    "กระเป๋าปะ-จีบBox-จีบคว่ำ-ทรงสี่เหลี่ยม-A4=1-01.svg",
    "กระเป๋าปะ-ทรงแหลม-A4=1-01.svg",
    "กระเป๋าปะ-ทรงโค้ง-A4=1-01.svg",
    "กระเป๋าปะ-ทรงหกเหลี่ยม-A4=1-01.svg",
    "กระเป๋าปะ-ทรงสี่เหลี่ยม-A4=1-01.svg"
};
int numPocketFileList = sizeof( pocketFileList ) / sizeof( pocketFileList[0] );
            ...
            ._(List()
                ...
                .onValueChanged( [=]( DecWidget &widget ) {
                    if( widget.value() == -1 ) {
                        ...
                        int pid = _product_id_list[(int)w];
                        SkDebugf( "Product ID: %i\n", pid );
                        SkDebugf( "SKU: %s\n", _sku_list[pid-1].c_str() );
                        setProduct( (int)pid, _sku_list[(int)pid-1].c_str() );
                    }
                })
            )
        )
            ...
            ._( Pages() 
                ...
                ._(List()
                    ...
                    .onValueChanged( [=]( DecWidget &widget ) {
                        ...
                        char sz[256];
                        strcpy( sz, "_magicpattern/V2-Blouse-Dress-SVG/" );
...
void DecSVGEditor::setProduct( int id, const char *sku ) {
    ...
    if( _product_id == 3719 ) {
        SkDebugf( "POCKET: %i, %s\n", id, sku );
        _sku = std::string( "A0001" );
        SkDebugf( "SKU: %s\n", sku );
        _vProductFileList.clear();
        SkDebugf( "OK1\n" );
        _pProductFileList->clear();
        SkDebugf( "OK2\n" );
        for( int i=0; i<numPocketFileList; i++ ) {
            _pProductFileList->_( ListItem( pocketFileList[i] ) );
            _vProductFileList.push_back( pocketFileList[i] );
        }
...
std::string DecSVGEditor::exportPDF(bool a4) {
    std::string fn = std::string("_export2/") + _filebasename;
    if( a4 ) {
        fn = fn + "-A4";
    }
    if( _is_S && _is_M && _is_L && _is_XL && _is_XXL ) {
...
bool DecSVGEditor::__jobPartExport(DecWidget &widget) {
    _pSKUList->select_listItem( __jobProductIndex );
...
bool DecSVGEditor::__jobProductExportByList(DecWidget &widget) {
    ...
    if( size_id == "all" ) {
        ...
        _is_XXL = true;      
        _forceBlack = false;
        _pSKUList->select_listItem( _pid2index_list[atoi( ptr )] );
        ...
        _pSKUList->select_listItem( _pid2index_list[atoi( ptr )-1] );
        updateNodeVisibility();
        exportProductPDF();
    }
...
void DecSVGEditor::exportProductPDF( bool cover, bool force ) {
    _pProductFileList->select_listItem( -1 );
    ...
    if( _isTiledExport ) {
        if( size_name == "COVER" ) {
            fn = std::string("_export2/") + std::string(szPID) + "-" + size_name + "-" + _sku + "-" + ".pdf";
        }
        else {
            fn = std::string("_export2/") + std::string(szPID) + "-A4-" + _sku + "-" + size_name + ".pdf";
        }
    }
    else {
        fn = std::string("_export2/") + std::string(szPID) + "-A1-" + _sku + "-"   + size_name + ".pdf";
    }
...
void DecSVGEditor::overlayImage( SkCanvas &canvas ) {
    ...
    for( int i=0; i<_vProductImageList.size(); i++ ) {
        img[i] = new DecImage();
        SkDebugf( "Load image: %s\n",_vProductImageList[i].c_str() );
...
void DecSVGEditor::overlayLayout( SkCanvas &canvas ) {
    ...
    if( n ) {
        char fn1[64];
        sprintf( fn1, "_magicpattern/Layout/Lay%02i.png", n );
        SkDebugf( "Load image 0: %s\n", fn1 );
        img[0] = new DecImage();
        img[0]->load( fn1 );   
        imgs_h += img[0]->image->height(); 
    }
    else {
        SkDebugf( "Layout image 0 not found: %s\n", _sku.c_str() );
    }
    ...
    if( img[0] ) {
        ...
        SkDebugf( "No Layout: %i %s\n", _product_id, _sku.c_str() );
    }

    if( img[1] ) {
        SkRect rect = SkRect::MakeXYWH( 0, 0, img[1]->image->width(), img[1]->image->height() );
        canvas.save();
    ...
    if( img[2] ) {
        SkRect rect = SkRect::MakeXYWH( 0, 0, img[2]->image->width(), img[2]->image->height() );
        canvas.save();
...
void DecSVGEditor::overlayCoverImage( SkCanvas &canvas ) {
    ...
    char sz[128];
    sprintf( sz, "_export_png2/%04i-0.png", _product_id );
    ...
    SkPaint paint;
    SkRect rect = SkRect::MakeXYWH( 0.2 * 72, 1.77 * 72, 4.75 * 72, (4.75 * 72) * h / w );
...
bool DecSVGEditor::checkSKUMaterial( std::string sz0, std::string sz1, std::string sz2, std::string sz3, 
        std::string sz5, std::string sz6, std::string sz7 )
 {
    if (sz0.find(_sku[0]) == std::string::npos) return false;
    if (sz1.find(_sku[1]) == std::string::npos) return false;
    if (sz2.find(_sku[2]) == std::string::npos) return false;
    if (sz3.find(_sku[3]) == std::string::npos) return false;
    if (sz5.find(_sku[5]) == std::string::npos) return false;
    if (sz6.find(_sku[6]) == std::string::npos) return false;
    if (sz7.find(_sku[7]) == std::string::npos) return false;
    return true;
}
...
void DecSVGEditor::overlayMaterial( SkCanvas &canvas ) {
    ...
    rect = SkRect::MakeXYWH( 0.2 * 72, yy * 72, 3.9 * 72, (3.9 * 72) * h / w );
    canvas.drawImageRect( img->image, rect, SkSamplingOptions(), &paint ); 
...
bool DecSVGEditor::__jobFileExport( DecWidget &widget ) {
    ...
    if( i == -2 ) {
        ...
        filepath = std::string("_magicpattern/V2-Blouse-Dress-SVG/") + _vProductFileList[i];
        ...
        if( (_vProductFileList[i][0] == 'C' || _vProductFileList[i][0] == 'K') &&
            (_vProductFileList[i][1] == 'P' || _vProductFileList[i][1] == 'B') &&
            (_vProductFileList[i][3] == 'N' || _vProductFileList[i][3] == 'S') &&
            (_vProductFileList[i][4] == '-')) {
            patternName = std::string(szProductID) + "แพทเทิร์นตัวเดรส ";
        }        
...
bool DecSVGEditor::__jobPNGFileExport( DecWidget &widget ) {
    ...
    SkDebugf( "------ __jobPNGFileExport %s\n", szPID );

    if (!_vProductFileList.size()) {
        SkDebugf( "----- NO PRODUCT LIST\n" );
        return false;
    }
    ...
    {
        i = __jobPNGFileIndex;
        filepath = std::string("_magicpattern/V2-Blouse-Dress-SVG/") + (_vProductFileList.size() > i ? _vProductFileList[i]:"");
    ...
    std::string fn;
    if( _is_png_forweb ) {
        fn = std::string("resources/_export_png_web/") + std::string(szPID) + "-" + std::string(szFI) + ".png";
    }
    else {
        fn = std::string("resources/_export_png/") + std::string(szPID) + "-" + size_name + "-" + std::string(szFI) + "-" + _filebasename  + ".png";
    }
    SkDebugf( "---- Check PNG: %s\n", fn.c_str() );
    if( !std::filesystem::exists( std::filesystem::path( fn ) ) ) {
        SkDebugf( "PNG: %s\n", fn.c_str() );
        int dpi = 72;
        if( _is_png_forweb ) {
...
void DecSVGEditor::init() {
    ...
    _pComboBoxTargetPaper->select_comboboxItem( 1 );

    fDom = pSVGDomEditor->fDom;

    std::filesystem::path fullpath( "_magicpattern/V2-Blouse-Dress-SVG/HBAN-A1=2-01.svg" );
...
int DecSVGEditor::getProductCode( int _type, int _length, int _hem, int _sleeve, int _neck, int _collar) {
...
void DecSVGEditor::generateProductCodeList() {
    ...
    _pSKUList->_( ListItem( "Pocket" ) );   
    _product_id_list[3718] = 3719;
    _sku_list[3718] = "POCKET10";       
    ...
    while( true ) {
        ...
        int _body = _generateLength * 15 + _generateType * 8 + _generateNeck;
        int _length = (_body / 15);
        int _type = (_body % 15) / 8;
        int _neck = (_body % 15) % 8;  
        ...
        int _productCode = getProductCode( _type, _length, _hem, _sleeve, _neck, _collar);
        _product_id_list[_generateCount-1] = _productCode;
        _sku_list[_productCode-1] = _sku;
        _pid2index_list[_productCode-1] = _generateCount-1;

        char sz[32];
        sprintf( sz, "%04i %s", _productCode, _sku.c_str() );
        //SkDebugf( "%s\n", sz );
                    ...
                    if( _generateHem >= 3 || (_generateHem >= 1 && _generateLength > 2)) {
                        ...
                        if( _generateLength >= 5 ) {
                            ...
                            if( _generateType >= 2 ) {
                                ...
                                if( std::filesystem::exists("mp2_index2pid.json") == false ) {
                                    SkDebugf( "Generated mp2_index2pid.json\n" );
                                    SkFILEWStream index2pid("mp2_index2pid.json");
                                    index2pid.write("[ ", strlen("[ ") );
                                    for( int i=0; i<3718; i++ ) {
                                        char sz[64];
                                        sprintf( sz, "%i,", _product_id_list[i] );
                                        index2pid.write( sz, strlen(sz) );
                                        if( i > 0 && (i % 50) == 0 ) {
                                            index2pid.write( "\n", strlen("\n") );
                                        }
                                    }
                                ...
                                if( std::filesystem::exists("mp2_pid2sku.json") == false ) {
                                    SkDebugf( "Generated mp2_pid2sku.json\n" );
                                    SkFILEWStream pid2sku("mp2_pid2sku.json");
                                    pid2sku.write("[ ", strlen("[ ") );
                                    for( int i=0; i<3718; i++ ) {
                                        char sz[64];
                                        sprintf( sz, "\"%s\",", _sku_list[i].c_str() );
                                        pid2sku.write( sz, strlen(sz) );
                                        if( i > 0 && (i % 20) == 0 ) {
                                            pid2sku.write( "\n", strlen("\n") );
                                        }
                                    }
                                ...
                                if( std::filesystem::exists("mp2_sku2pid.json") == false ) {
                                    SkDebugf( "Generated mp2_sku2pid.json\n" );
                                    SkFILEWStream sku2pid("mp2_sku2pid.json");
                                    sku2pid.write("{ ", strlen("{ ") );
                                    for( int i=0; i<3718; i++ ) {
                                        char sz[64];
                                        sprintf( sz, "\"%s\":%i,", _sku_list[i].c_str(), i );
                                        sku2pid.write( sz, strlen(sz) );
                                        if( i > 0 && (i % 20) == 0 ) {
                                            sku2pid.write( "\n", strlen("\n") );
                                        }
                                    }
