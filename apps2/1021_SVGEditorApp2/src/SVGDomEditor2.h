...
public:    
    ...
    SkSVGNode* selectNodeByPosition(float x, float y, float scale, SkSVGNode* pSelectedNode = NULL, bool bInboundSelectedOnly = false);
    ...
    void exportPDF( const char *filename );
    //SkPixmap getPixmap( float dpi, std::function<void(SkCanvas &canvas)>hook=nullptr );
    //sk_sp<SkData> getPNGData( float dpi );
    sk_sp<SkData> getPDFData();
    //sk_sp<SkData> getSVGData();
    //sk_sp<SkData> getSVGRenderData();
    pesBuffer getPNGBuffer( float dpi );
    pesBuffer getPDFBuffer();
    pesBuffer getSVGBuffer();
    pesBuffer getSVGRenderBuffer();
    void startExportPDF( );
