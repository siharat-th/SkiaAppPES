...
// fork from http-server v14.1.1 (http-server/bin/http-server)
// - add 'return server;' in function listen() { ...; return server;  }
// - add promise on portfinder.getPort()
// - export WebServerPromise promise (return server from listen())
'use strict';
...
if (require.main === module) {
  process.title = 'web-server.js';  
}

if (argv.h || argv.help) {
  console.log([
    'usage: '+process.title+' [path] [options]',
    '',
    'options:',
...
const WebServerPromise = (async () => {
  if (!port) {
    ...
    //delay and resolve promise on next ticks.
    //cause: wait for other scripts binded event success before server emit events.
    
    // use Promise and regular setTimeout
    //await new Promise((res, rej) => setTimeout(res, 0));    
    
    // or use timer-promise from 'node:timers/promises'
    await setTimeout(0, '');
  }

  if(!port) {
    logger.info('Could not get a free port');
    process.exit();
  }
...
function listen(port) {
  ...
  if (tls) {
    options.https = {
      cert: argv.C || argv.cert || path.join(__dirname, 'cert.pem'),
      key: argv.K || argv.key || path.join(__dirname, 'key.pem'),
  ...
  // see: https://nodejs.org/dist/latest/docs/api/net.html#serverlistenport-host-backlog-callback
  server.listen(port, host, function () {
    ...
    logger.info([chalk.yellow('\n'+process.title+' version: '), chalk.cyan(require('./package.json').version)].join(''));
    logger.info([chalk.yellow('http-server version: '), chalk.cyan(require('http-server/package.json').version)].join(''));
...
exports.WebServerPromise = WebServerPromise;
